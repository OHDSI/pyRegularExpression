...
@pytest.mark.parametrize(
    ...
    [
        # Positive examples
        (
            "Ten participants were lost to follow-up during the trial.",
            True,
            "v1_pos_lost_to_follow_up",
        ),
        (
            "Three patients withdrew consent after enrollment.",
            True,
            "v1_pos_withdrew_consent",
        ),
        (
            "Five subjects dropped out due to adverse events.",
            True,
            "v1_pos_dropped_out",
        ),
        # Trap examples
        (
            "Screen failures were excluded before randomization.",
            False,
            "v1_trap_screen_failures",
        ),
        (
            "Patients were censored at transplant or death.",
            False,
            "v1_trap_censored_event",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        # Positive examples
        (
            "Ten patients dropped out during follow-up due to side effects.",
            True,
            "v2_pos_during_followup",
        ),
        (
            "Seven participants withdrew consent during the study.",
            True,
            "v2_pos_during_study",
        ),
        # Trap examples and negatives
        (
            "Participants were excluded at screening.",
            False,
            "v2_trap_screening_excluded",
        ),
        (
            "Dropped out due to insurance issues before enrollment.",
            False,
            "v2_trap_before_enrollment",
        ),
        (
            "Lost contact with some participants post-study.",
            False,
            "v2_neg_after_study_context",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Results:\nFive participants withdrew consent during follow-up.",
            False,
...
# ─────────────────────────────
# Lighter Tests for v4 – Cue + context + numeric evidence
# (e.g., "10 participants", "5%")
# ─────────────────────────────
@pytest.mark.parametrize(
    "text, should_match, test_id",
    [
        # Positive examples
        (
            "Ten participants were lost to follow-up during the study.",
            True,
            "v4_pos_count_during_study",
        ),
        (
            "5% of patients withdrew consent during follow-up.",
            True,
            "v4_pos_percent_with_context",
        ),
        # Negative examples
        ("Withdrew consent due to side effects.", False, "v4_neg_no_numeric"),
        (
            "Lost to follow-up noted in the chart.",
            False,
            "v4_neg_no_context_or_number",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Withdrew consent during follow-up due to adverse events.",
            True,
            "v5_pos_withdrew_during",
        ),
        # Negative examples
        (
            "Patients lost their ID cards during follow-up.",
            False,
            "v5_neg_irrelevant_use_lost",
        ),
        (
            "Consent withdrawn prior to randomization.",
            False,
            "v5_neg_pre_randomization",
        ),
    ],
)
