...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Enrollment criteria included patients with asthma.",
            True,
            "v1_pos_enrollment_criteria_included",
        ),
        (
            "Participants were not eligible if they refused consent.",
            True,
            "v1_pos_not_eligible_if",
        ),
        (
            "Exclusion criteria included those with renal disease.",
            True,
            "v1_pos_exclusion_criteria_included",
        ),
        (
            "Eligibility criteria were recently updated.",
            True,
            "v1_pos_eligibility_criteria_phrase",
        ),
        # More negative traps
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            "v3_pos_combo_heading",
        ),
        (
            "Eligible patients were aged 18–65.\n\nEligibility Criteria:\n(no content)",
            False,
            "v3_neg_cue_before_heading",
        ),
        (
            "Patient selection:\nEligible subjects were assessed.",
            False,
            "v3_neg_heading_not_matched",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Eligible patients were included if they had diabetes and we excluded those with prior insulin use.",
            True,
            "v4_pos_incl_near_excl",
        ),
        (
            "Patients were eligible; those with heart disease were excluded.",
            True,
            "v4_pos_semicolon_separation",
        ),
        ("Eligible patients had COPD.", False, "v4_neg_no_exclusion"),
        (
            "We excluded those with cancer but did not specify inclusion criteria.",
            False,
            "v4_neg_exclusion_only",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Adults 18–65 with diabetes were eligible; prior insulin use was an exclusion criterion.",
            True,
            "v5_pos_full_template",
        ),
        (
            "Children 6–12 were eligible; those with seizures were excluded.",
            True,
            "v5_pos_children_range",
        ),
        (
            "Eligible patients were included if they had diabetes.",
            False,
            "v5_neg_soft_template",
        ),
        (
            "Adults aged 45–75 were recruited; exclusion criteria not stated.",
            False,
            "v5_neg_missing_exclusion_cue",
        ),
    ],
)
