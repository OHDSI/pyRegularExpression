...
- Functional checks for v3, v4, and v5 (block-based, covariate-aware,
  and template-based)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "The model controlled for comorbidities and smoking status.",
            True,
            "v1_pos_controlled",
        ),
        # Traps and negatives
        (
            "Drug dose was adjusted based on renal function.",
            False,
            "v1_trap_dose_adjustment",
        ),
        (
            "Treatment regimens were adjusted according to response.",
            False,
            "v1_trap_treatment_adjustment",
        ),
        (
            "Baseline covariates were listed but no adjustment applied.",
            False,
            "v1_neg_baseline_listing",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        # Positive
        (
            "Outcomes were adjusted for age, BMI, and smoking.",
            True,
        ...
        (
            "Adjusted values are reported in Supplementary Table 1.",
            False,
            "v2_neg_generic_adjusted",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            "v3_pos_statistical_analysis",
        ),
        (
            "Covariate adjustment:\nThe analysis controlled for smoking and BMI.\n\n",
            True,
            "v3_pos_covariate_adjustment",
        ),
        # Negative: adjustment cue but outside heading block
        (
            "Results:\nHazard ratios were adjusted for age and sex.",
            False,
            "v3_neg_wrong_section",
        ),
        (
            "Baseline characteristics:\nPatients’ age and BMI were recorded.",
            False,
            "v3_neg_baseline_only",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            "v4_pos_with_covariates",
        ),
        (
            "The hazard ratios were controlled for baseline BMI.",
            True,
            "v4_pos_with_bmi",
        ),
        # Negative
        ...
        (
            "Values were adjusted with respect to dose response.",
            False,
            "v4_trap_dose_response",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        # Positive
        (
            "Hazard ratios adjusted for age, BMI, and smoking.",
            True,
            "v5_pos_hr_adjusted",
        ),
        (
            "Multivariable model adjusted including age and sex.",
            True,
            "v5_pos_multivariable_template",
        ),
        # Negative
        (
            "The patient lost weight adjusted for baseline measures.",
            False,
            "v5_neg_irrelevant_use",
        ),
