...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "We examined drug exposure in the target cohort.",
            True,
            "v1_pos_examined_drug_exposure",
        ),
        (
            "Radiation exposure was analyzed over 12 months.",
            True,
            "v1_pos_radiation_exposure",
        ),
        (
            "Exposure to opioids was tracked post-discharge.",
            True,
            "v1_pos_exposure_opioids_tracked",
        ),
        # ❌ Traps or irrelevant mentions
        (
            "Participants were allocated to the exposure group.",
            False,
            "v1_neg_exposure_group_label",
        ),
        (
            "Exposure pathway analysis was done post-hoc.",
            False,
            "v1_neg_exposure_as_analysis_type",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Exposure was defined as two prescriptions of ACE inhibitors.",
            True,
            "v2_pos_defined_as_two_prescriptions",
        ),
        (
            "We operationalized opioid exposure based on prescription records.",
            True,
            "v2_pos_operationalized_opioid_exposure",
        ),
        (
            "Exposure was considered present if >=1 refill was observed.",
            True,
            "v2_pos_considered_present",
        ),
        # ❌ Missing defining verb near exposure term
        (
            "Exposure was common among participants.",
            False,
            "v2_neg_exposure_without_definition",
        ),
        (
            "We measured exposure but did not define it.",
            False,
            "v2_neg_exposure_mention_only",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Exposure was defined as ≥2 prescriptions within 90 days.",
            True,
            "v4_pos_defined_plus_threshold",
        ),
        (
            "We considered exposure present if patients had at least 30 days' supply.",
            True,
            "v4_pos_considered_with_criterion",
        ),
        (
            "Operationalized exposure as 1 refill in 14 days.",
            True,
            "v4_pos_operationalized_refill",
        ),
        # ❌ No numeric or time-based definition
        (
            "Exposure was defined as drug use.",
            False,
            "v4_neg_defined_but_no_criteria",
        ),
        (
            "We examined exposure but did not define thresholds.",
            False,
            "v4_neg_no_thresholds_given",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Exposure was defined as ≥2 prescriptions of metformin within 60 days.",
            True,
            "v5_pos_tight_defined_metformin",
        ),
        (
            "Exposure = 3 fills of opioid within 30 days prior to index.",
            True,
            "v5_pos_tight_equals_style",
        ),
        # ❌ Looser or implicit definitions
        ("We examined opioid exposure during follow-up.", False, "v5_neg_generic_mention"),
        (
            "Exposure was based on prescription activity.",
            False,
            "v5_neg_loose_definition",
        ),
    ],
)
