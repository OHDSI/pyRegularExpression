...
@pytest.mark.parametrize(
    ...
    [
        (
            "The study included an interim analysis after 6 months.",
            True,
            "v1_pos_basic",
        ),
        ("A DSMB meeting was scheduled.", True, "v1_pos_dsmb"),
        (
            "Final analysis was performed after all data was collected.",
            False,
            "v1_neg_no_interim",
        ),
        (
            "This analysis did not involve any early stopping.",
            False,
            "v1_neg_explicit_no_stop",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Interim analysis at 6 months with O’Brien-Fleming boundary p < 0.001.",
            True,
            "v2_pos_boundary",
        ),
        ...
        (
            "Interim analysis but no statistical criteria mentioned.",
            False,
            "v2_neg_no_boundary",
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Interim analysis at 6 months with O’Brien-Fleming boundary p < 0.001.",
            True,
            "v3_pos_full_template",
        ),
        (
            "Interim analysis at 12 weeks using Haybittle-Peto p < 0.01.",
            True,
            "v3_pos_alt_template",
        ),
        (
            "Interim analysis with DSMB but no boundaries given.",
            False,
            "v3_neg_missing_boundary",
        ),
