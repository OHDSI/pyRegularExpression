...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Subjects not eligible if they had uncontrolled hypertension.",
            True,
            "v1_pos_not_eligible_if",
        ),
        (
            "Exclusion criteria include pregnancy and lactation.",
            True,
            "v1_pos_criteria_include",
        ),
        (
            "Must not have received prior radiation therapy.",
            True,
            "v1_pos_must_not_have",
        ),
        (
            "Participants were excluded only if they failed screening.",
            True,
            "v1_pos_excluded_only_if",
        ),
        # Negative: Traps and non-rule statements
        (
            "Twenty participants dropped out during follow-up.",
            False,
            "v1_neg_dropout_trap",
        ),
        (
            "The analysis excluded patients with incomplete data.",
            False,
            "v1_neg_analysis_excluded",
        ),
        (
            "We excluded variables from regression modeling.",
            False,
            "v1_neg_variable_excluded",
        ),
        (
            "The trial excluded the possibility of bias by blinding.",
            False,
            "v1_neg_abstract_exclusion",
        ),
        (
            "Subjects were included in the final safety cohort.",
            False,
            "v1_neg_inclusion_mislead",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Patients were excluded if they had liver cirrhosis.",
            True,
            "v2_pos_excluded_if",
        ),
        ("Subjects not eligible: history of seizure.", True, "v2_pos_colon_list"),
        (
            "Must not have prior chemotherapy if enrolled.",
            True,
            "v2_pos_must_not_have_if",
        ),
        (
            "Patients were excluded only if they failed liver panel tests.",
            True,
            "v2_pos_excluded_only",
        ),
        (
            "Exclusion criteria: allergy to contrast agents.",
            True,
            "v2_pos_colon_exclusion",
        ),
        # Negative: No gating token near cue
        ("Subjects were excluded due to clinician discretion.", False, "v2_neg_no_gate"),
        (
            "Patients excluded from the interim safety analysis.",
            False,
            "v2_neg_analysis_trap",
        ),
        (
            "Exclusion was considered unlikely by the team.",
            False,
            "v2_neg_no_true_cue",
        ),
        (
            "Subjects not eligible because of subjective reasons.",
            False,
            "v2_neg_because_not_token",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "PRIMARY EXCLUSION CRITERIA:\nPatients excluded for poor adherence.",
            False,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Participants must not have diabetes if enrolled in phase II.",
            True,
            "v4_pos_must_not_have_if",
        ),
        # Negative: v2 passes but no conditional verb
        (
            "Subjects were excluded if they were under 18.",
            False,
            "v4_neg_no_conditional_verb",
        ),
        (
            "Not eligible due to uncontrolled hypertension.",
            False,
            "v4_neg_no_gate_and_conditional",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Exclusion criteria: uncontrolled diabetes or recent MI.",
            True,
            "v5_pos_colon_template",
        ),
        (
            "Patients were excluded if they had prior stroke.",
            True,
            "v5_pos_were_excluded_if",
        ),
        # Negative: Template doesn't match tightly
        (
            "Patient was excluded if diagnosed with epilepsy.",
            False,
            "v5_neg_singular_was_excluded",
        ),
        (
            "Subjects must not have HIV to be eligible.",
            False,
            "v5_neg_not_a_template",
        ),
    ],
)
