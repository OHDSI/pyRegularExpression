...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Participants were required to have a diagnosis of hypertension.",
            True,
            "v1_pos_required_to_have",
        ),
        (
            "Subjects were eligible if they were aged between 18 and 65 years.",
            True,
            "v1_pos_eligible_if",
        ),
        (
            "Inclusion criteria: male, aged 50–75 years, BMI <30.",
            True,
            "v1_pos_colon_criteria",
        ),
        (
            "To be included, patients must have completed at least one cycle.",
            True,
            "v1_pos_must_have",
        ),
        (
            "Exclusion criteria also included pregnancy or lactation.",
            True,
            "v1_pos_finds_keyword_in_exclusion_context",
        ),
        ...
        (
            "The study included 200 participants from five centers.",
            False,
            "v1_neg_trap_study_included",
        ),
        (
            "Patients included in this analysis were observed for safety.",
            False,
            "v1_neg_trap_included_in_analysis",
        ),
        (
            "Our analysis included adjustment for baseline covariates.",
            False,
            "v1_neg_trap_analysis_included",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Participants were eligible if they had one cardiovascular event.",
            True,
            "v2_pos_eligible_if",
        ),
        (
            "Subjects included only those with chronic kidney disease.",
            True,
            "v2_pos_included_only",
        ),
        (
            "To be eligible, a patient must have a signed consent form.",
            True,
            "v2_pos_fixed_multi_word_gate",
        ),
        (
            "Patients are eligible and must have an ECOG status of 0-1.",
            True,
            "v2_pos_must_have_as_gate_2",
        ),
        (
            "Inclusion criteria: patients with stage III cancer.",
            True,
            "v2_pos_fixed_colon_gate",
        ),
        # Negative Examples: No match if gating token is absent or logic fails.
        (
            "Eligible subjects were adults aged 18 to 65 years.",
            False,
            "v2_neg_no_gating_token",
        ),
        (
            "All included patients provided informed consent.",
            False,
            "v2_neg_no_gating_token_2",
        ),
        (
            "Inclusion criteria were broad and generally inclusive.",
            False,
            "v2_neg_no_colon_gate",
        ),
        (
            "This was a study about eligible patient populations.",
            False,
            "v2_neg_cue_without_gate",
        ),
        (
            "The analysis included patients if they were enrolled before 2020.",
            False,
            "v2_neg_trap_analysis_included",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "eligibility criteria\n- Participants must have completed one vaccine dose.",
            True,
            "v3_pos_lowercase_heading",
        ),
        # Negative: Cue is outside a recognized heading block.
        (
            "A patient must be eligible.\n\nInclusion criteria:\n(no rule here)",
            False,
            "v3_neg_cue_before_block",
        ),
        (
            "PRIMARY INCLUSION CRITERIA:\nPatients must have failed prior therapy.",
            False,
            "v3_neg_unrecognized_heading",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Eligible if they must have completed surgical resection.",
            True,
            "v4_pos_must_have_if",
        ),
        # Negative: v2 match but without the required strong verb.
        (
            "Participants were eligible if they were over 18.",
            False,
            "v4_neg_no_strong_verb",
        ),
        (
            "Subjects required to have ECOG status ≤ 1.",
            False,
            "v4_neg_no_gating_word",
        ),  # Fails v2 check
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Inclusion criteria: adults aged 18–65, non-smokers.",
            True,
            "v5_pos_colon_list",
        ),
        (
            "Patients were eligible if they exhibited symptoms of depression.",
            True,
            "v5_pos_plural_were_eligible_if",
        ),
        # Negative: Do not match if template is violated.
        (
            "Patient was eligible if creatinine clearance > 50 mL/min.",
            False,
            "v5_neg_singular_was_eligible",
        ),
        (
            "Eligible patients had to have hypertension to enroll.",
            False,
            "v5_neg_had_to_have_not_template",
        ),
    ],
)
