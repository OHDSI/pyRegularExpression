"""
risk_of_bias_assessment_finder.py – p/r ladder for *risk‑of‑bias*.

Five variants (v1–v5):
    • v1 – high recall: any sentence with a bias‑assessment cue ("risk of
      bias", ROBINS‑I, ROB 2, Cochrane tool, Newcastle–Ottawa Scale, NOS,
      quality assessment).
    • v2 – v1 **and** assessment verb (assessed, evaluated, rated, scored,
      used, applied) within ±4 tokens of the cue.
    • v3 – only inside a *Risk of Bias / Quality Assessment* heading block
      (first ≈400 characters).
    • v4 – v2 plus explicit rating keyword (low, high, moderate, unclear)
      **or** tool name in the same sentence.
...
TOOL_RE = re.compile(
    r"\b(?:ROBINS[-– ]?I|ROB[-– ]?2|Cochrane\s+(?:risk\s+of\s+bias\s+)?tool|"
    r"Newcastle[-–]Ottawa\s+Scale|NOS)\b",
    re.I,
)
BIAS_CUE_RE = re.compile(r"\brisk\s+of\s+bias|quality\s+assessment\b", re.I)
VERB_RE = re.compile(
    r"\b(?:assessed|evaluated|rated|scored|used|applied|performed)\b", re.I
)
RATING_RE = re.compile(r"\b(?:low|high|moderate|unclear)\b", re.I)
HEAD_ROB_RE = re.compile(
    r"(?im)^(risk\s+of\s+bias|quality\s+assessment|study\s+quality)\s*[:\-]?",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"risk\s+of\s+bias\s+was\s+assessed[^\.\n]{0,60}(?:ROBINS[- ]?I|ROB[- ]?2|"
    r"Newcastle)",
    re.I,
)
...
def find_risk_of_bias_assessment_v2(text: str, window: int = 4):
    ...
    for patt in [BIAS_CUE_RE, TOOL_RE]:
        for m in patt.finditer(text):
            w_s, w_e = _char_to_word((m.start(), m.end()), spans)
            if any(
                abs(v - w_s) <= window or abs(v - w_e) <= window
...
def find_risk_of_bias_assessment_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_ROB_RE.finditer(text)
    ]
...
RISK_OF_BIAS_ASSESSMENT_FINDERS: dict[
...
__all__ = [
    ...
    "RISK_OF_BIAS_ASSESSMENT_FINDERS",
]
