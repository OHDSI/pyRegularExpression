"""
exclusion_rule_finder.py – p/r ladder for *exclusion‑rule* statements.
...
Five variants (v1–v5):
    ...
    • v4 – v2 plus explicit conditional verbs (must NOT have), excludes
      follow‑up / dropout traps
    • v5 – tight template: “Exclusion criteria: …” or “Patients were excluded
      if …”
...
EXCLUSION_RULE_TERM_RE = re.compile(
    r"\b(?:exclusion\s+criteria|excluded(?:\s+only|\s+if)?|not\s+eligible|"
    r"must\s+not\s+have|history\s+of|allergy|exclusions\s*[:\n]?)\b",
    re.I,
)

GATING_TOKEN_RE = re.compile(
    r"\b(?:if|only|criteria:|:|not\s+eligible|must\s+not\s+have)\b", re.I
)

HEADING_EXCLUSION_RE = re.compile(r"^(exclusion criteria|exclusions)\s*[:\n]?", re.I)

TRAP_RE = re.compile(
    r"\b(excluded\s+variables?|excluded\s+the\s+possibility|(?:analysis|study|"
    r"trial)\s+excluded|withdrew|withdrawn|lost\s+to\s+follow[- ]?up|dropped\s+"
    r"out|after\s+enrol(?:l|l)ment|during\s+follow[- ]?up|excluded\s+from\s+"
    r".*analysis)\b",
    re.I,
)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:exclusion\s+criteria:\s+[^\.\n]{0,120}|patients?\s+were\s+excluded\s+"
    r"if\s+[^\.\n]{0,120}|participants?\s+were\s+not\s+eligible\s+if\s+"
    r"[^\.\n]{0,120})",
    re.I,
)

CONDITIONAL_VERB_RE = re.compile(
    r"\b(?:must\s+not\s+have|were\s+not\s+eligible|had\s+to\s+be\s+free\s+of)\b",
    re.I,
)
...
def find_exclusion_rule_v1(text: str) -> list[tuple[int, int, str]]:
    """Tier 1 – high recall: any exclusion/‘not eligible’ cue."""
    token_spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
...
def find_exclusion_rule_v4(text: str) -> list[tuple[int, int, str]]:
    """Tier 4 – v2 + explicit negative conditional verbs."""
    token_spans = _token_spans(text)
    tokens = [text[s:e].lower() for s, e in token_spans]
    out = []
    ...
    for i in range(len(tokens) - 2):
        if (
            tokens[i] == "must"
            and tokens[i + 1] == "not"
            and tokens[i + 2] in {"have", "be", "meet"}
        ):
            # look ahead for conditional token
            if any(
                t in {"if", "only", "unless", "provided"}
                for t in tokens[i + 3 : i + 10]
            ):
                w_s, w_e = i, i + 3
