"""
inclusion_rule_finder.py – p/r ladder for *inclusion‑rule* statements.
...
Five variants (v1–v5):
    ...
    • v4 – v2 plus explicit conditional verbs (must have / required to have),
      excludes traps
    • v5 – tight template: “Inclusion criteria: …” or “Patients were eligible
      if …”
...
INCL_TERM_RE = re.compile(
    r"\b(?:inclusion\s+criteria|included|eligible|must\s+have|required\s+to\s+"
    r"have|included\s+only|included\s+if)\b",
    re.I,
)

GATING_TOKEN_RE = re.compile(r"\b(?:if|only|criteria:|:|must\s+have)", re.I)

HEADING_INCL_RE = re.compile(
    r"(?m)^(?:inclusion\s+criteria|eligibility\s+criteria|inclusion)\s*[:\-]?"
    r"[ \t]*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:study\s+included|analysis\s+included|included\s+patients|patients\s+"
    r"included)\b",
    re.I,
)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:inclusion\s+criteria:\s+[^\.\n]{0,120}|patients?\s+were\s+eligible\s+"
    r"if\s+[^\.\n]{0,120})",
    re.I,
)
...
def find_inclusion_rule_v2(text: str, window: int = 5):
    ...
    for g_match in GATING_TOKEN_RE.finditer(text):
        w_s, w_e = _char_span_to_word_span(
            (g_match.start(), g_match.end()), token_spans
        )
...
def find_inclusion_rule_v4(text: str, window: int = 6):
    ...
    conditional_verb_re = re.compile(
        r"\b(?:must\s+have|required\s+to\s+have|had\s+to\s+have|must\s+possess)\b",
        re.I,
    )
    token_spans = _token_spans(text)
    ...
    for c_match in conditional_verb_re.finditer(text):
        w_s, w_e = _char_span_to_word_span(
            (c_match.start(), c_match.end()), token_spans
        )
        for i in range(w_s, w_e + 1):
            cond_idx.add(i)
