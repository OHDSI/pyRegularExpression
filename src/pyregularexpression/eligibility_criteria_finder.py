"""
eligibility_criteria_finder.py – p/r ladder for eligibility criteria.

Five variants (v1–v5):
    • v1 – high recall: any eligibility cue (eligible patients were, inclusion
      criteria included, we excluded participants who, exclusion criteria,
      criteria for enrollment, eligible if).
    • v2 – explicit inclusion OR exclusion cue + condition within ±4 tokens
      (numeric/age/diagnosis etc.).
    • v3 – only inside an *Eligibility / Inclusion and Exclusion Criteria*
      heading block (first ~500 characters).
    • v4 – v2 plus both an inclusion and an exclusion cue in the same
      sentence/nearby to maximise precision.
    • v5 – tight template: paired statement listing age/diagnosis eligibility
      and exclusion of specific conditions (e.g., “Adults 18–65 with diabetes
      were eligible; prior insulin use was an exclusion”).
...
def _char_span_to_word_span(
    span: tuple[int, int], token_spans: Sequence[tuple[int, int]]
) -> tuple[int, int]:
    s_char, e_char = span
...
INCL_CUE_RE = re.compile(
    r"\b(eligible\s+(patients|participants|individuals)|inclusion\s+criteria"
    r"(?:\s+included)?|patients\s+were\s+eligible)\b",
    re.I,
)

EXCL_CUE_RE = re.compile(
    r"\b(?:were\s+excluded|we\s+excluded|exclusion\s+criteria(?:\s+included)?"
    r"|not\s+eligible\s+if|excluded\s+if|were\s+not\s+eligible(?:\s+if)?)\b",
    re.I,
)

QUALIFIER_RE = re.compile(
    r"\b(aged\s+\d{1,3}(?:\s*(?:–|-|to|\u2013|\u2014|\u2212)\s*)\d{1,3}|"
    r"diagnosed\s+\w+|with\s+(?:a\s+)?\w+|history\s+of\s+\w+|had\s+\w+|"
    r"those\s+with\s+\w+|patients\s+with\s+\w+)\b",
    re.I,
)

ELIG_CUE_RE = re.compile(
    r"\b(?:inclusion\s+criteria|exclusion\s+criteria|eligibility\s+criteria|"
    r"enrollment\s+criteria|eligible)\b",
    re.I,
)

HEADING_ELIG_RE = re.compile(
    r"(?m)^\s*(?:eligibility\s+criteria|inclusion\s+and\s+exclusion\s+"
    r"criteria|study\s+population|participants?)\s*[:\-]?\s*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:diagnostic\s+criteria|classification\s+criteria|performance\s+"
    r"criteria)\b",
    re.I,
)

TIGHT_TEMPLATE_RE = re.compile(
    r"\b(?:(?:adults?|children|patients|participants)\s+\d{1,3}(?:\s*(?:–|-|to)"
    r"\s*)\d{1,3}|eligible\s+(?:patients|participants|individuals)[^\.\n]{0,100}"
    r"?(?:aged\s+\d{1,3}(?:\s*(?:–|-|to)\s*)\d{1,3}))(?:[^\.\n]{0,100}?"
    r"(?:inclusion\s+criteria|were\s+eligible))?(?:[^\.\n]{0,100}?"
    r"(?:exclusion\s+criteria|were\s+excluded|not\s+eligible|excluded\s+if))?",
    re.I,
)
...
def find_eligibility_criteria_v2(
    text: str, window: int = 10
) -> list[tuple[int, int, str]]:
    token_spans = _token_spans(text)
...
def find_eligibility_criteria_v3(
    text: str, block_chars: int = 500
) -> list[tuple[int, int, str]]:
...
def find_eligibility_criteria_v4(text: str) -> list[tuple[int, int, str]]:
    """
    v4 – fires if the text contains a valid inclusion cue *before* a valid
    exclusion cue. Returns a single span from the start of the first
    inclusion match to the end of the first exclusion match.
    """
    token_spans = _token_spans(text)
...
def find_eligibility_criteria_v5(text: str) -> list[tuple[int, int, str]]:
    """Tier 5 – v tight template match for age range + eligibility + exclusion."""
    token_spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
...
# ─────────────────────────────
ELIGIBILITY_CRITERIA_FINDERS: dict[
