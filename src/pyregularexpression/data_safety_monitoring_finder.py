"""
data_safety_monitoring_finder.py – p/r ladder for *Data‑Safety Monitoring*.

Five variants (v1–v5):
    • v1 – high recall: any sentence with DSMB/DMC cues ("Data Safety
      Monitoring Board", "Data Monitoring Committee", DSMB, DMC, periodic safety
      review).
    • v2 – v1 **and** safety verb (reviewed, monitored, met, evaluated) within
      ±4 tokens of the cue.
    • v3 – only inside a *Data Safety Monitoring / DSMB / DMC* heading block
      (first ≈400 characters).
    • v4 – v2 plus explicit independence/frequency term (independent,
      external, quarterly, periodic) or phrase “safety data”.
    • v5 – tight template: “An independent DSMB met quarterly to review
      adverse events.”
...
DSMB_RE = re.compile(
    r"(?:\bindependent\s+)?(?:data\s+(?:and\s+)?safety\s+monitoring\s+"
    r"(?:board|committee)|data\s+monitoring\s+committee|DSMB|DMC)",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:reviewed|monitored|met|evaluated|assessed)\b", re.I
)
SAFETY_RE = re.compile(r"\b(?:safety\s+data|adverse\s+events?|AEs?)\b", re.I)
FREQ_RE = re.compile(
    r"\b(?:quarterly|monthly|periodic(?:ally)?|annual(?:ly)?)\b", re.I
)
TRAP_RE = re.compile(
    r"\bmonitoring\s+of\s+data\s+quality|data\s+safety\s+sheet\b", re.I
)
HEAD_DSMB_RE = re.compile(
    r"(?m)^(?:data\s+safety\s+monitoring|DSMB|DMC)(?:\s*[:\-].*)?$", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"independent\s+(?:DSMB|DMC)\s+met(?:\s+\w+)?\s+to\s+review\s+"
    r"(?:safety\s+data|adverse\s+events?)",
    re.I,
)
...
def find_data_safety_monitoring_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_DSMB_RE.finditer(text)
    ]
...
def find_data_safety_monitoring_v4(text: str, window: int = 6):
    ...
    extra_idx = {
        ...
        if SAFETY_RE.search(t) or FREQ_RE.search(t)
    }
...
DATA_SAFETY_MONITORING_FINDERS: dict[
...
__all__ = [
    ...
    "DATA_SAFETY_MONITORING_FINDERS",
]
