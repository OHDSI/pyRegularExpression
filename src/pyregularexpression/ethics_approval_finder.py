"""
ethics_approval_finder.py – p/r ladder for *ethics approval & consent*.

Five variants (v1–v5):
    • v1 – high recall: any IRB/ethics/consent keyword (IRB, ethics
      committee, informed consent).
    • v2 – approval verb (approved, reviewed, waived) within ±4 tokens of an
      IRB/ethics keyword.
    • v3 – only inside an *Ethics approval / Ethical considerations* heading
      block.
    • v4 – v2 plus explicit consent phrase or IRB protocol number pattern to
      avoid generic ethics principles.
    • v5 – tight template: “Protocol approved by XYZ IRB #2021‑45; informed
      consent obtained.”
...
# ─────────────────────────────
IRB_RE = re.compile(
    r"\b(?:irb|institutional\s+review\s+board|research\s+ethics\s+board|reb|"
    r"ethics\s+committee|ethics\s+review\s+board)\b",
    re.I,
)
APPROVAL_VERB_RE = re.compile(
    r"\b(?:approved|reviewed|waived|granted|obtained|cleared)\b", re.I
)
CONSENT_RE = re.compile(
    r"\b(?:informed\s+consent|written\s+consent|verbal\s+consent|parental\s+"
    r"consent)\b",
    re.I,
)
IRB_NUM_RE = re.compile(r"\b(?:protocol|project)?\s?#?\d{2,4}[-_]?[A-Za-z]?\d{0,3}\b")
HEADING_ETHICS_RE = re.compile(
    r"(?m)^(?:ethics(?:\s+approval)?|ethical\s+considerations|ethics\s+"
    r"statement|informed\s+consent)\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\bethical\s+principles|ethical\s+guidelines|ethically\s+conducted\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"protocol\s+(?:was\s+)?approved\s+by\s+[^\.\n]{0,60}(?:irb|ethics\s+"
    r"committee)[^\.\n]{0,80}(?:informed\s+consent\s+(?:was\s+)?(?:obtained|"
    r"waived))",
    re.I,
)
...
def find_ethics_approval_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snip in matches:
        if any(
            c_s - window <= w_s <= c_e + window or w_s - window <= c_s <= w_e + window
