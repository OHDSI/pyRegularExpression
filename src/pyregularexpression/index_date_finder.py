"""
index_date_finder.py – p/r ladder for *index‑date definition* statements.
...
Functions return a list of tuples:
...
DEFINE_VERB_RE = re.compile(
    r"\b(?:defined|set|assigned|taken|established|determined|calculated)\b", re.I
)

EQUAL_SYNTAX_RE = re.compile(r"=", re.I)

HEADING_INDEX_RE = re.compile(
    r"(?m)^(?:index\s+date|baseline\s+date|time\s+zero)\b.*$", re.I
)

TRAP_RE = re.compile(
    r"\b(?:index\s+(?:case|test|patient|event)|follow(?:ed|ing)?\s+from|"
    r"data\s+entry)\b",
    re.I,
)
...
def find_index_date_v2(text: str, window: int = 5):
    ...
    verb_idx = {
        i for i, t in enumerate(tokens) if DEFINE_VERB_RE.fullmatch(t.rstrip(".;,"))
    }
    out: list[tuple[int, int, str]] = []
...
def find_index_date_v4(text: str, window: int = 5):
    ...
    tokens = [text[s:e] for s, e in token_spans]
    index_matches = [
        (m.start(), m.end(), m.group(0)) for m in INDEX_TERM_RE.finditer(text)
    ]
    out: list[tuple[int, int, str]] = []
    for start, end, snippet in index_matches:
        ...
        if (
            EQUAL_SYNTAX_RE.search(context_text)
            or DEFINE_VERB_RE.search(context_text)
        ) and not TRAP_RE.search(context_text):
...
def find_index_date_v5(text: str):
    ...
    template_re = re.compile(
        r"(?:index\s+date|baseline\s+date)\s*(?:=|was\s+defined\s+as|was\s+set\s+"
        r"as|was\s+assigned\s+as)\s+[^\.\;\n]{0,50}",
        re.I,
    )
