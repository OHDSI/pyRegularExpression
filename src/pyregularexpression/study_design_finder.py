"""
study_design_finder.py – p/r ladder for *study design* declarations.

Five variants (v1–v5):

    • v1 – high recall: any design keyword (cohort, case‑control, randomized
      controlled trial, etc.)
    • v2 – design keyword preceded by linking phrase ("was a", "this was",
      “conducted as”) within ±window tokens
    • v3 – only inside a *Study design / Methods / Study type* heading block
    • v4 – v2 plus canonical design pair (e.g., “randomized controlled
      trial”, “prospective cohort study”) or design + temporal qualifier
      (prospective/retrospective)
    • v5 – tight template: “Retrospective cohort study using registry data”,
      “Randomized controlled trial of drug X”, etc.
...
DESIGN_KEYWORD_RE = re.compile(
    r"\b(?:randomi(?:s|z)ed\s+controlled\s+trial|rct|cohort\s+study|prospective"
    r"\s+cohort|retrospective\s+cohort|case[- ]?control\s+study|cross[- ]?"
    r"sectional\s+study|nested\s+case[- ]?control|case\s+series|case\s+report|"
    r"ecological\s+study|registry\s+study)\b",
    re.I,
)

LINK_PHRASE_RE = re.compile(
    r"\b(?:was|were|is|study\s+was|this\s+was|conducted\s+as)\b", re.I
)

CANONICAL_PAIR_RE = re.compile(
    r"\b(?:randomi(?:s|z)ed\s+controlled\s+trial|prospective\s+cohort\s+study|"
    r"retrospective\s+cohort\s+study|case[- ]?control\s+study|cross[- ]?"
    r"sectional\s+study)\b",
    re.I,
)

HEADING_DESIGN_RE = re.compile(
    r"(?m)^(?:study\s+design|methods?|design)\s*[:\-]?\s*$", re.I
)

TRAP_RE = re.compile(r"\bdesign(?:ed)?\s+to\b", re.I)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:prospective|retrospective|randomi(?:s|z)ed|cross[- ]?sectional|"
    r"case[- ]?control|cohort)\s+(?:cohort\s+)?study|randomi(?:s|z)ed\s+"
...
def find_study_design_v2(text: str, window: int = 4) -> list[tuple[int, int, str]]:
    ...
    for m in DESIGN_KEYWORD_RE.finditer(text):
        ...
        if any(
            link_index
            for link_index in link_idx
            if w_s - window <= link_index <= w_e + window
        ):
            out.append((w_s, w_e, m.group(0)))
    return out


def find_study_design_v3(
    text: str, block_chars: int = 300
) -> list[tuple[int, int, str]]:
    token_spans = _token_spans(text)
    blocks = []
...
def find_study_design_v4(text: str, window: int = 5) -> list[tuple[int, int, str]]:
    ...
    for w_s, w_e, snippet in matches:
        sentence = text[
            max(0, text.rfind(".", 0, w_s)) : text.find(".", w_e) + 1 or len(text)
        ]
