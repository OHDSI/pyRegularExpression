"""
background_rationale_finder.py – precision/recall ladder for
*study background / rationale* statements.

Five variants (v1–v5):
    • v1 – high recall: any gap/rationale cue (e.g., “prior studies have
      shown”, “however, little is known”, “important gap”, “need for this
      study”, “to address this”).
    • v2 – sentence containing ≥2 gap phrases **or** a gap phrase plus
      “study/this study” within ±4 tokens.
    • v3 – only inside an *Introduction / Background* heading block (first
      ~500 chars after heading).
    • v4 – v2 plus unmet‑need phrases like “little is known”, “unknown”,
      “not well understood”, or explicit “rationale”. Filters generic facts.
    • v5 – tight template: statement with contrast connector (yet/however)
      + unmet‑need phrase (little is known/unknown) + motivation
...
GAP_PHRASE_RE = re.compile(
    r"\b(?:prior\s+studies\s+have\s+shown|however,?\s+little\s+is\s+known"
    r"|little\s+is\s+known|important\s+gap|knowledge\s+gap"
    r"|evidence\s+is\s+limited|unknown|not\s+well\s+understood"
    r"|need\s+for\s+this\s+study|to\s+address\s+(?:this|these)\b"
    r"|rationale\s+for)\b",
    re.I,
)
STUDY_TOKEN_RE = re.compile(
    r"\b(?:this\s+study|the\s+study|study|research|work|investigation)\b", re.I
)
...
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:however|yet)[\s,;:]{0,5}[^\.\n]{0,100}?(?:little\s+is\s+known"
    r"|unknown|not\s+well\s+understood)[^\.\n]{0,100}?(?:therefore|thus"
    r"|to\s+address\s+this|this\s+study\s+aims)",
    re.I,
)
RATIONALE_RE = re.compile(
    r"\b(little\s+is\s+known|not\s+well\s+understood|unknown|knowledge\s+gap"
    r"|important\s+gap)\b",
    re.I,
)
UNMET_RE = re.compile(
...
def find_background_rationale_v2(
    text: str, window: int = 40
...
def find_background_rationale_v4(
    text: str, window: int = 40
) -> list[tuple[int, int, str]]:
    """
