"""
covariate_adjustment_finder.py – p/r ladder for *covariate adjustment*.

Five variants (v1–v5):
    • v1 – high recall: any adjustment/control cue or multivariable model
      mention.
    • v2 – adjustment cue + linking token (“for”, “with”) within ±window
      tokens.
    • v3 – only inside a *Statistical analysis / Covariate adjustment*
      heading block.
...
HEADING_ADJ_RE = re.compile(
    r"(?m)^(?:statistical\s+analysis|covariate\s+adjustment|analytical\s+"
    r"approach)\s*[:\-]?\s*$",
    re.I,
)
...
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:adjusted|controlling|controlled)\s+for\s+[A-Za-z0-9,\s]+(?:age|sex|bmi|"
    r"smok)\w*[^\.\n]{0,40}|multivaria(?:ble|te)\s+model\s+(?:including|"
    r"adjusted)\s+[^\.\n]{0,60}",
    re.I,
)
