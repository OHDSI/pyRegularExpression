"""
exposure_definition_finder.py – p/r ladder for *exposure definition*.

Five variants (v1–v5):

    • v1 – high recall: any exposure cue
    • v2 – cue + defining verb (defined/considered/operationalized) within
      ±window tokens
    • v3 – only inside an *Exposure definition / Exposure assessment* heading
      block
    • v4 – v2 plus explicit criterion token (≥, at least, prescriptions,
      days, etc.), excludes generic mentions
    • v5 – tight template: “Exposure was defined as ≥2 prescriptions…”,
      “Exposure = drug X for 30 days”, etc.
...
DEFINE_VERB_RE = re.compile(
    r"\b(define|defined|consider|considered|operationalize|operationalized|"
    r"required|required|present)\b",
    re.I,
)

CRITERION_TOKEN_RE = re.compile(
    r"\b(?:>=|<=|>|<|at\s+least|more\s+than|\d+\s*(?:prescriptions?|doses?|"
    r"fills?|days?|weeks?|months)|within\s+\d+)\b",
    re.I,
)

HEADING_EXPOSURE_RE = re.compile(
    r"(?m)^(?:exposure\s+(?:definition|assessment|classification))\s*[:\-]?\s*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:occupational\s+exposure|environmental\s+exposure|randomi(?:s|z)ed|"
    r"exposure\s+group|exposure\s+pathway)\b",
    re.I,
)

NEGATION_RE = re.compile(
    r"\b(not|no|never|did\s+not|was\s+not|were\s+not|cannot|can't|won't)\b", re.I
)
...
def find_exposure_definition_v2(
    text: str, window: int = 8
) -> list[tuple[int, int, str]]:
    """Tier 2 – cue + defining verb within ±window tokens, excluding negated."""
    token_spans = _token_spans(text)
    tokens = [text[s:e] for s, e in token_spans]
    out: list[tuple[int, int, str]] = []
...
def find_exposure_definition_v3(
    text: str, block_chars: int = 400
) -> list[tuple[int, int, str]]:
    """Tier 3 – allow exposure threshold expressions inside heading blocks."""
    token_spans = _token_spans(text)
...
def find_exposure_definition_v4(
    text: str, window: int = 6
) -> list[tuple[int, int, str]]:
    """Tier 4 – cue + defining verb + numeric/time criterion in window."""
    token_spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
    ...
    for m in EXPOSURE_DEF_CUE_RE.finditer(text):
        ...
        w_s, w_e = _char_span_to_word_span((cue_start, cue_end), token_spans)
        w_lo = max(0, w_s - window)
        w_hi = min(len(token_spans), w_e + window + 1)

        window_text = text[token_spans[w_lo][0] : token_spans[w_hi - 1][1]]
        if DEFINE_VERB_RE.search(window_text) and CRITERION_TOKEN_RE.search(
            window_text
        ):
            out.append((w_s, w_e, m.group(0)))
    return out
...
# ─────────────────────────────
EXPOSURE_DEFINITION_FINDERS: dict[
