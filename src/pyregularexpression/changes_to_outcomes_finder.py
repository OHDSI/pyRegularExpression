"""changes_to_outcomes_finder.py – precision/recall ladder for *changes to
prespecified outcomes after trial initiation*.

Five variants (v1–v5):
    • v1 – high recall: any modification cue (changed the primary outcome,
           added a new secondary outcome, amended the outcomes, outcome was revised).
    • v2 – modification cue + outcome keyword within ±4 tokens and temporal phrase
           (mid-study, after trial started, X months into the trial).
    • v3 – only inside a *Protocol amendments / Outcome changes* heading block
           (first ~400 chars).
    • v4 – v2 plus explicit reason phrase (due to, because of, owing to)
           nearby for extra precision.
    • v5 – tight template: “Due to low event rate, the primary outcome was
           changed from OS to DFS midway.”
...
HEADING_CHG_RE = re.compile(
    r"(?m)^(?:[ \t]*outcome\s+changes?|changes\s+to\s+outcomes?|protocol\s+amendments?)"
    r"\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\bchanges?\s+in\s+outcomes?|significant\s+change\s+in\s+outcome\s+values?\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"\b(?:due\s+to|because\s+of|owing\s+to)\s+[^\.\n]{0,60}?primary\s+outcome\s+was\s+"
    r"changed\s+from\s+[^\.\n]{0,40}?\s+to\s+[^\.\n]{0,40}?\s*(?:mid[- ]?(?:study|way)|"
    r"after\s+\d+\s+events)\b",
    re.I,
)
...
def _collect(patterns: Sequence[re.Pattern[str]], text: str):
    ...
    for patt in patterns:
        for m in patt.finditer(text):
            print(
                f"[DEBUG] Regex matched: '{m.group(0)}' at span {m.start()}–{m.end()}"
            )
...
def find_changes_to_outcomes_v2(text: str, window: int = 4):
    ...
    temp_matches = [
        (m.start(), m.end(), m.group()) for m in TEMPORAL_RE.finditer(text)
    ]
    out = []
...
# mapping
CHANGES_TO_OUTCOMES_FINDERS: dict[
