"""
missing_data_handling_finder.py – p/r ladder for *missing‑data handling*.

Five variants (v1–v5):
    • v1 – high recall: any missing‑data cue (missing data were, imputed,
      multiple imputation, complete‑case analysis, LOCF, last observation
      carried forward) in a sentence.
    • v2 – v1 **and** analysis verb (imputed, handled, performed, used,
      applied) within ±4 tokens of cue.
    • v3 – only inside a *Missing Data / Imputation* heading block (first ~400
      characters).
    • v4 – v2 plus explicit technique keyword (multiple imputation, chained
      equations, MICE, LOCF, complete‑case, maximum likelihood) in same
      sentence.
    • v5 – tight template: “Missing covariates were imputed using chained
      equations (mice).”
...
MISS_CUE_RE = re.compile(
    r"\b(?:missing\s+data|imputed|imputation|complete[- ]case|last\s+observation"
    r"\s+carried\s+forward|locf|mice|multiple\s+imputation)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:imputed|handled|performed|used|applied|conducted)\b", re.I
)
TECH_RE = re.compile(
    r"\b(?:multiple\s+imputation|chained\s+equations|mice|locf|last\s+"
    r"observation\s+carried\s+forward|complete[- ]case|maximum\s+likelihood|"
    r"inverse\s+probability\s+weighting|pattern\s+mixture)\b",
    re.I,
)
HEAD_MISS_RE = re.compile(
    r"(?m)^(?:missing\s+data|imputation|handling\s+of\s+missing|data\s+"
    r"imputation)\s*[:\-]?\s*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"missing[^\.\n]{0,20}imputed[^\.\n]{0,40}(?:chained\s+equations|mice|"
...
MISSING_DATA_HANDLING_FINDERS: dict[
