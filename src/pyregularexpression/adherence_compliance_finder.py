"""adherence_compliance_finder.py – precision/recall ladder for *treatment
adherence / compliance* metrics. Five variants (v1–v5):

    • v1 – high recall: any sentence with an adherence cue ("adherence",
      "compliance", medication possession ratio, proportion of days covered, MPR,
      PDC, pill count).
    • v2 – v1 **and** analytic verb (defined, calculated, measured, assessed,
      evaluated) within ±4 tokens of the cue.
    • v3 – only inside an *Adherence / Compliance* heading block (first ≈400
      characters).
    • v4 – v2 plus explicit numeric threshold or metric keyword (≥80 %, ≥0.8,
      MPR, PDC, pill count) in the same sentence.
...
ADH_CUE_RE = re.compile(
    r"\b(?:adherence|compliance|medication\s+possession\s+ratio|mpr"
    r"|proportion\s+of\s+days\s+covered|pdc|pill\s+counts?)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:defined|calculated|measured|assessed|evaluated|determined|computed)\b", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"adherence\s+was\s+defined[^\.\n]{0,60}(?:pdc|mpr)[^≥>]*[≥>]\s*0?\.?(?:7|8|80)",
    re.I,
)
TRAP_RE = re.compile(
    r"\b(?:adherence\s+to\s+(?:guidelines|study\s+procedures|protocols?)"
    r"|baseline\s+adherence|expected\s+adherence)\b",
    re.I,
)
HEAD_ADH_RE = re.compile(
    r"(?m)^(?:adherence|compliance|medication\s+adherence)\s*[:\-]?", re.I
)
THRESH_RE = re.compile(
    r"(?:pdc|mpr|pill\s*counts?)\s*[≥>]\s*\d+(?:\.\d+)?(?:\s*(?:%|percent))?"
    r"|[≥>]\s*\d+(?:\.\d+)?(?:\s*(?:%|percent|proportion|ratio))",
    re.I,
)
