"""data_linkage_method_finder.py – p/r ladder for *data‑linkage methods*.

Five variants (v1–v5):
    • v1 – high recall: sentence mentioning linkage cues ("records were linked",
      "data linkage", "linked files", master patient index) regardless of context.
    • v2 – v1 **and** object keywords (records/data/files/registries/datasets)
      within ±3 tokens of “linkage/linked”.
    • v3 – only inside a *Data Linkage / Record Linkage / Matching* heading
      block (first ≈400 characters).
    • v4 – v2 plus explicit method term (probabilistic, deterministic, exact
      match, fuzzy match, hashed, master patient index, MPI).
    • v5 – tight template: “Hospital admissions were probabilistically linked to
      death‑registry data using date of birth and NHS number.”
...
LINK_CUE_RE = re.compile(
    r"\b(?:linkage|linked|linking|match(?:ed|ing)?)\b", re.I)
OBJ_RE = re.compile(
    r"\b(?:records?|data(?:sets?)?|files?|registries|registry|databases?)\b",
    re.I)
METHOD_RE = re.compile(
    r"\b(?:probabilistic(?:ally)?|deterministic(?:ally)?|exact\s+match|"
    r"fuzzy\s+match|hashed|token[- ]?based|master\s+patient\s+index|MPI)\b",
    re.I,
)
HEAD_LINK_RE = re.compile(
    r"(?mi)^(?:data\s+linkage|record\s+linkage|data\s+matching|"
    r"linkage\s+method)\b.*",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:probabilistic(?:ally)?\s+linked\s+.+?registry|deterministic(?:ally)?"
    r"\s+match(?:ed)?)",
    re.I,
)
TRAP_RE = re.compile(
    r"\blink\s+between|link\s+to\s+outcome|hyperlink|website\s+link\b", re.I
)
...
def find_data_linkage_method_v2(text: str, window: int = 3):
    ...
    for link_index in link_idx:
        if any(abs(o - link_index) <= window for o in obj_idx):
            w_s, w_e = _char_to_word(spans[link_index], spans)
            out.append((w_s, w_e, tokens[link_index]))
    return out
