"""
limitations_finder.py – p/r ladder for *study limitations* sections.

Five variants (v1–v5):
    • v1 – high recall: any limitations cue (limitations of this study,
      limitation, potential bias, small sample) in a sentence.
    • v2 – v1 **and** explicit self‑reference (this study/our study) or phrase
      “we acknowledge” near the cue.
    • v3 – only inside a *Limitations* or *Strengths and Limitations* heading
      block (first ~400 characters).
    • v4 – v2 plus at least one weakness keyword (bias, power, sample,
      generalisability, confounding) in the same sentence.
    • v5 – tight template: “Limitations include small sample and short
      follow‑up.”
...
LIMIT_CUE_RE = re.compile(
    r"\b(?:limitations?|limitation|bias|small\s+sample|underpowered)\b", re.I
)
SELF_REF_RE = re.compile(
    r"\b(?:this\s+study|our\s+study|we\s+acknowledge|we\s+recognise)\b", re.I
)
WEAKNESS_RE = re.compile(
    r"\b(?:bias|small\s+sample|underpowered|not\s+powered|short\s+follow[- ]up|"
    r"confound(?:ing|ers?)|generalisa?bility|selection\s+bias)\b",
    re.I,
)
HEAD_LIMIT_RE = re.compile(
    r"(?mi)^(?:limitations|strengths?\s+and\s+limitations)\b.*", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"limitations?\s+include[s]?\s+[^\".\n]{0,80}(?:sample|follow[- ]up|bias)",
    re.I,
)
TRAP_RE = re.compile(
    r"\blimitations?\s+of\s+(?:previous|prior|other)\s+studies\b", re.I
)
