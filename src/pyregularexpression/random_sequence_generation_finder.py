"""
random_sequence_generation_finder.py – p/r ladder for *random sequence*.

Five variants (v1–v5):
    • v1 – high recall: any generation cue (computer-generated, random number
      table, coin toss, shuffled envelopes, block randomization).
    • v2 – generation cue + randomisation keyword (sequence, allocation,
      randomization) within ±4 tokens.
    • v3 – within a *Randomisation / Sequence Generation* heading block.
    • v4 – v2 plus explicit method modifier (block, stratified, permuted,
      envelopes) nearby.
    • v5 – tight template: “Allocation sequence computer-generated using block
...
GEN_CUE_RE = re.compile(
    r"\b(?:computer[- ]?generated|computerised|computerized|random\s+number\s+"
    r"table|coin\s+toss|shuffled\s+(?:opaque\s+)?envelopes?|sealed\s+opaque\s+"
    r"envelopes?|permuted\s+block|block\s+randomi[sz]ation|stratified\s+"
    r"randomi[sz]ation)\b",
    re.I,
)
RAND_KEY_RE = re.compile(
    r"\b(?:randomi[sz]ation|randomi[sz]ed|allocation|sequence)\b", re.I
)
METHOD_MOD_RE = re.compile(
    r"\b(?:block|blocks?|permuted|stratified|opaque\s+envelopes?|shuffled)\b",
    re.I,
)
HEADING_RAND_RE = re.compile(
    r"(?m)^(?:randomi[sz]ation|sequence\s+generation|allocation\s+sequence)\s*"
    r"[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\brandom(?:ly)?\s+(?:assigned|selected)|random\s+sampling|random\s+"
    r"effects?\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:the\s+)?allocation\s+sequence(?:\s+was)?\s+computer[- ]?generated(?:\s+"
    r"\w+){0,10}?\s+block\s+randomi[sz]ation",
    re.I,
)
...
RANDOM_SEQUENCE_GENERATION_FINDERS: dict[
