...
TIGHT_TEMPLATE_RE = re.compile(r"allocation\s+sequence\s+computer[- ]?generated[^\.\n]{0,80}?block\s+randomi[sz]ation", re.I)
...
def find_random_sequence_generation_v2(text:str, window:int=4):
    ...
    gen_idx={i for i,t in enumerate(tokens) if GEN_CUE_RE.fullmatch(t)}
    out=[]
    for i in gen_idx:
