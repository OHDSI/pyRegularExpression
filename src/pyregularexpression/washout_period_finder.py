"""
washout_period_finder.py – p/r ladder for *washout period* definitions.

Five variants (v1–v5):
    • v1 – high recall: any washout/run‑in/drug‑free cue
    • v2 – cue + explicit duration (months / weeks / years) or “drug‑free /
      treatment‑free” within ±window tokens
    • v3 – only inside a *Washout period / Clearance period / Run‑in* heading
      block
    • v4 – v2 plus temporal anchor before index/baseline (before / prior to /
      preceding), excludes side‑effect stoppage traps
    • v5 – tight template: “12‑month washout with no antihypertensives”,
      “patients were drug‑free for 6 months before index”, etc.
...
WASHOUT_CUE_RE = re.compile(
    rf"""\b(
        washout(?:{SEP}period)?        |   # "washout", "washout-period"
        run{SEP}in                      |   # "run in", "run-in"
        clearance(?:{SEP}period)?      |   # "clearance", "clearance-period"
...
BEFORE_ANCHOR_RE = re.compile(
    r"\b(?:before|prior\s+to|preceding|pre[- ]?index|pre[- ]?baseline)\b", re.I
)
...
TRAP_RE = re.compile(
    r"\b(?:stopped|discontinued|due\s+to\s+side[- ]?effects|adverse\s+events?)\b",
    re.I,
)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:\d+[- ]?(?:month|week|year)s?\s+washout(?:\s+period)?\s+with\s+no\s+"
    r"[a-z\s]{1,40}(?:\s+was\s+required|\s+was\s+implemented|\s+completed)?|"
    r"drug[- ]?free\s+for\s+\d+[- ]?(?:month|week|year)s?\s+before\s+index)",
    re.I,
)
...
def _char_to_token_index_map(
    text: str, token_spans: list[tuple[int, int]]
) -> dict[int, int]:
...
def find_washout_period_v3(
    text: str, block_chars: int = 500
) -> list[tuple[int, int, str]]:
    """Tier 3 – match any duration or cue inside heading blocks."""
    token_spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
    ...
    for heading_match in HEADING_WASHOUT_RE.finditer(text):
        ...
        for m in list(WASHOUT_CUE_RE.finditer(block)) + list(
            DURATION_RE.finditer(block)
        ):
            if TRAP_RE.search(m.group(0)):
                continue
...
def find_washout_period_v4(text: str, window: int = 8) -> list[tuple[int, int, str]]:
    ...
    for cue_match in WASHOUT_CUE_RE.finditer(text):
        ...
        for dur_match in DURATION_RE.finditer(text):
            ...
            snippet = text[
                min(cue_match.start(), dur_match.start()) : max(
                    cue_match.end(), dur_match.end()
                )
                + 40
            ]
