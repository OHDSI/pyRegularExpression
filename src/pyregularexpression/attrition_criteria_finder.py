"""attrition_criteria_finder.py – precision/recall ladder for *attrition criteria*
(post‑enrolment loss).
Five variants (v1–v5):
    • v1 – high recall: any attrition cue (lost / withdrew / dropped out)
    • v2 – cue + study‑context token within ±window tokens (during follow‑up /
      during study)
    • v3 – only inside an *Attrition / Loss to follow‑up* heading block
    • v4 – v2 plus numeric evidence (count or %), excludes exit‑criterion and
      pre‑screen traps
    • v5 – tight template: “N participants were lost to follow‑up”,
      “withdrew consent during follow‑up”, etc.
Each function returns a list of tuples: (start_token_idx, end_token_idx,
matched_snippet)
"""
...
ATTRITION_CUE_RE = re.compile(
    r"\b(?:lost\s+to\s+follow[- ]?up|lost\s+contact|withdrew\s+consent|withdrawn|"
    r"dropped\s+out|discontinued\s+participation|lost)\b",
    re.I,
)
...
STUDY_CONTEXT_RE = re.compile(
    r"""
    \b(
        during\s+(the\s+)?(study|follow[-\s]?up|trial)|
        (study|trial|follow[-\s]?up)\s+period|
        in\s+(the\s+)?(study|trial|follow[-\s]?up)
    )\b
    """,
...
NUMERIC_EVIDENCE_RE = re.compile(
    r"""
    (?:(?:\d+|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|
    eighteen|nineteen|twenty)\s+(participants?|patients?|subjects?)|\d+\s*%
    (\s+of\s+(participants?|patients?|subjects?))?|n\s*=\s*\d+
    )
    """,
    re.IGNORECASE | re.VERBOSE,
)

HEADING_ATTRITION_RE = re.compile(
    r"(?m)^(?:attrition|loss\s+to\s+follow[- ]?up|participant\s+flow)\s*[:\-]?\s*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:exclusion\s+criteria|excluded\s+if|exit\s+when|censored|screen\s+failure|"
    r"pre[- ]?randomi[sz]ation)\b",
    re.I,
)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:\d+\s*(?:participants?|patients?|subjects?)\s+(?:were\s+)?"
    r"(?:lost\s+to\s+follow[- ]?up|withdrew\s+consent|dropped\s+out)|"
...
def find_attrition_criteria_v2(
    ...
    cue_matches = [
        ...
        if not TRAP_RE.search(text[max(0, m.start() - 30) : m.end() + 30])
    ]
...
def find_attrition_criteria_v3(
    ...
    for h in HEADING_ATTRITION_RE.finditer(text):
        ...
        nxt_blank = text.find("\n\n", start)
            ...
            nxt_blank
            if 0 <= nxt_blank - start <= block_chars
...
def find_attrition_criteria_v4(
    text: str, window: int = 6
) -> list[tuple[int, int, str]]:
...
# ─────────────────────────────
ATTRITION_CRITERIA_FINDERS: dict[
