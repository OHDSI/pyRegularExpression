"""
funding_statement_finder.py – p/r ladder for *study funding statements*.

Five variants (v1–v5):
    • v1 – high recall: any funding cue ("funded by", "supported by",
      "sponsored by", funding source, grant number, NIH, Wellcome Trust).
    • v2 – v1 **and** a funding verb (funded, supported, sponsored, provided
      by) within ±4 tokens of the cue.
    • v3 – only inside a *Funding* or *Financial Support* heading block (first
      ≈400 characters).
    • v4 – v2 plus explicit grant/organisation identifier (e.g.,
      R01‑HL123456, grant number XXXXX, corporate name) in same sentence.
...
FUND_CUE_RE = re.compile(
    r"\b(?:funded|funding|supported|financially\s+supported|sponsored|"
    r"funding\s+source|grant(?:\s+number)?|grants?)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:funded|supported|support|sponsored|provided|awarded|made possible)\b",
    re.I,
)
GRANT_ID_RE = re.compile(
    r"\b(?:R\d{2}[A-Z]{0,2}\d{6}|[A-Z]{2,}-?\d{4,}|grant\s+\d{5,}|\d{6,})\b",
    re.I,
)
GRANT_RE = re.compile(r"\b(R\d{2}|U\d{2}|K\d{2})\s?[A-Z]{2,}\d{5}\b", re.I)
ORG_RE = re.compile(
    r"\b(?:NIH|National\s+Institutes\s+of\s+Health|NSF|Wellcome\s+Trust|"
    r"Gates\s+Foundation|Pfizer|Novartis|Merck|Roche)\b",
    re.I,
)
HEAD_FUND_RE = re.compile(
    r"(?m)^(?:funding|financial\s+support|sources?\s+of\s+funding|"
    r"acknowledg(?:e)?ments?)\s*[:\-]?\s*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"\bSupported by\s+(?:(?:[A-Z][A-Za-z]+(?: [A-Z][a-z]+)*)\s+grant\s+"
    r"(?:R\d{2}[- ]?[A-Z]{2,4}\d{6}|IIS[- ]?\d{6,7}|\d{6,})\s*(?:and\s+)*)+\b",
    re.I,
)
TRAP_RE = re.compile(
    r"\bno\\s+personal\\s+fees|conflicts?\\s+of\\s+interest|employed\\s+by\\b",
    re.I,
)
...
def find_funding_statement_v1(text: str) -> list[tuple[int, int, str]]:
    """Tier 1 – high recall: any funding cue, grant/org mention."""
...
def find_funding_statement_v2(text: str, window: int = 4):
    ...
    cue_idx = {
        ...
        if FUND_CUE_RE.search(t) or ORG_RE.search(t) or GRANT_ID_RE.search(t)
    }
...
def find_funding_statement_v3(text: str, block_chars: int = 400):
    spans = _token_spans(text)
    blocks = [
        (h.end(), min(len(text), h.end() + block_chars))
        for h in HEAD_FUND_RE.finditer(text)
    ]
...
def find_funding_statement_v4(text: str, window: int = 6):
    ...
    id_idx = {
        i
        for i, t in enumerate(tokens)
        if GRANT_ID_RE.search(t) or ORG_RE.search(t)
    }
    ...
    for w_s, w_e, snip in matches:
        ...
        if any(w_s - window <= k <= w_e + window for k in id_idx):
            ...
            if VERB_RE.search(snip) and (
                GRANT_ID_RE.search(snip) or ORG_RE.search(snip)
            ):
                out.append((w_s, w_e, snip))
...
FUNDING_STATEMENT_FINDERS: dict[
