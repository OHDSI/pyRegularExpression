"""
event_adjudication_finder.py – p/r ladder for *event‑adjudication*.

Five variants (v1–v5):
    • v1 – high recall: any sentence with adjudication cues ("events were
      adjudicated", "clinical events committee", CEC, "endpoint committee",
      blinded adjudicators).
    • v2 – v1 **and** object keyword (event/endpoint) within ±5 tokens of
      “adjudicat*”.
    • v3 – only inside an *Event Adjudication / Clinical Events Committee /
      Endpoint Committee* heading block (first ≈400 characters).
    • v4 – v2 plus explicit independence/blinding term (independent, blinded)
      **or** committee acronym (CEC, DSMB) in the same sentence.
    • v5 – tight template: “All MI events were independently adjudicated by a
      blinded CEC.”
...
COMM_RE = re.compile(
    r"\b(?:clinical\s+events?\s+committee|endpoint\s+committee|CEC|DSMB|DMC)\b",
    re.I,
)
BLIND_RE = re.compile(r"\b(?:blinded|independent(?:ly)?|masked)\b", re.I)
HEAD_ADJ_RE = re.compile(
    r"(?m)^(?:event\s+adjudication|clinical\s+events?\s+committee|endpoint\s+"
    r"committee)\s*[:\-]?\s*$",
    re.I,
)
...
TRAP_RE = re.compile(
    r"\blegal\s+adjudicat|court\s+adjudicat|dispute\s+adjudicat\b", re.I
)
...
def find_event_adjudication_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_ADJ_RE.finditer(text)
    ]
...
def find_event_adjudication_v4(text: str, window: int = 6):
    ...
    extra_idx = {
        ...
        if COMM_RE.fullmatch(t) or BLIND_RE.fullmatch(t)
    }
...
EVENT_ADJUDICATION_FINDERS: dict[
