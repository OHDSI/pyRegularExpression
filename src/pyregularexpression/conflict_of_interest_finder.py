"""conflict_of_interest_finder.py – precision/recall ladder for
*conflict‑of‑interest disclosures*.

Five variants (v1–v5):
    • v1 – high recall: sentence containing a conflict cue ("conflict of interest",
           "competing interests", disclosures, "no competing interests").
    • v2 – v1 **and** disclosure verb (declare, disclose, report, state) within
           ±4 tokens of the cue.
    • v3 – only inside a *Conflict of Interest / Disclosures / Competing Interests*
           heading block (first ≈400 characters).
    • v4 – v2 plus explicit company/payment or explicit negation phrase
           ("no competing interests", "no conflict") in same sentence.
...
COI_CUE_RE = re.compile(
    r"\b(?:conflicts?\s+of\s+interest|competing\s+interests?|conflict\s+disclosures?)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:declare(?:s|d)?|disclose(?:s|d)?|report(?:s|ed)?|state(?:s|d)?)\b", re.I
)
HEAD_COI_RE = re.compile(
    r"(?m)^(?:conflicts?\s+of\s+interest|competing\s+interests?|disclosures?)"
    r"\s*[:\-]?\s*$",
    re.I,
)
COMPANY_RE = re.compile(
    r"\b(?:Pfizer|Novartis|Merck|Roche|AstraZeneca|Bayer|GSK|Sanofi"
    r"|Johnson\s+&?\s*Johnson|Amgen|Lilly)\b",
    re.I,
)
NO_COI_RE = re.compile(r"\bno\s+(?:conflicts?|competing\s+interests?)\b", re.I)
TIGHT_TEMPLATE_RE = re.compile(
    r"authors?\s+declare\s+no\s+competing\s+interests", re.I
)
TRAP_RE = re.compile(
    r"\bconflict(?:ing)?\s+evidence|conflict\s+with\s+previous\s+studies\b", re.I
)
...
def find_conflict_of_interest_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_COI_RE.finditer(text)
    ]
...
CONFLICT_OF_INTEREST_FINDERS: dict[
