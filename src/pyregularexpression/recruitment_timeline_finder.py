"""
recruitment_timeline_finder.py – p/r ladder for *recruitment period*.

Five variants (v1–v5):
    • v1 – high recall: any recruitment/enrolment cue (recruited, enrolled,
      study period, follow‑up) immediately followed by a date or date‑range.
    • v2 – v1 **and** explicit date range separator (to, through, –, —) within
      the phrase.
    • v3 – only inside a *Recruitment / Study Period / Timeline* heading block
      (first ~500 characters).
    • v4 – v2 plus follow‑up duration phrase (followed for, follow‑up of X
      months/years) in the same sentence.
    • v5 – tight template: “Enrolled March 2015–July 2017; each followed
      12 months.”
...
DATE_RANGE_RE = (
    rf"(?:\b\d{{4}}\b|\b{MONTHS}\s+\d{{4}})\s*(?:–|-|—|to|through|until)\s*"
    rf"(?:\b\d{{4}}\b|\b{MONTHS}\s+\d{{4}})"
)
YEAR = r"(?:19|20)\d{2}"
DATE_RE = rf"(?:{MONTHS}\s+{YEAR}|{YEAR})"

ENROL_CUE_RE = re.compile(
    r"\b(?:recruit(?:ed|ment)|enrol(?:led|ment)|included|study\s+period|"
    r"data\s+collection|patients?\s+were\s+enrolled)\b",
    re.I,
)
FOLLOW_CUE_RE = re.compile(r"\bfollow(?:-?up|ed|ed\s+up|for)\b", re.I)
HEAD_RECRUIT_RE = re.compile(
    r"(?m)^(?:recruitment|enrolment|study\s+period|timeline)\s*[:\-]?", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    rf"(?:enrol(?:led|ment)|enrolled)\s+{DATE_RANGE_RE};?\s+(?:each\s+)?"
    r"(?:patients\s+were\s+)?(?:followed|follow\s*-?up\s*(?:was|for)?)\s+\d+\s+"
    r"(?:months?|years?)",
    re.I,
)
    ...
    for m in ENROL_CUE_RE.finditer(text):
        if inside(m.start()):
            for s, e in blocks:
                if s <= m.start() < e:
                    block_text = text[s:e]
                    if re.search(DATE_RANGE_RE, block_text) or re.search(
                        DATE_RE, block_text
                    ):
                        w_s, w_e = _char_to_word((m.start(), m.end()), spans)
                        out.append((w_s, w_e, m.group(0)))
...
RECRUITMENT_TIMELINE_FINDERS: dict[
