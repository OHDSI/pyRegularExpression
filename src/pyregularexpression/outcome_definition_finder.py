"""
outcome_definition_finder.py – p/r ladder for *outcome definition*.
...
Five variants (v1–v5):
    ...
    • v4 – v2 plus explicit criterion token (within X days, composite of,
      readmission, etc.), excludes vague listings
    • v5 – tight template: “Primary outcome: readmission within 30 days”,
      “Outcome was defined as death from any cause”, etc.
...
def _char_span_to_word_span(
    ...
    w_end = next(
        (i for i, (s, e) in reversed(list(enumerate(token_spans))) if s < e_char),
        len(token_spans) - 1,
    )
...
DEFINE_VERB_RE = re.compile(
    r"\b(?:defined|was|were|considered|designated|chosen|specified)\b", re.I
)
CRITERION_TOKEN_RE = re.compile(
    r"\b(?:within\s+\d+\s*(?:day|week|month|year)s?|\d+\s*(?:day|week|month|"
    r"year)s?|readmission|hospitalisation|death|mi|stroke|composite|"
    r"incidence|duration|rate)\b",
    re.I,
)
HEADING_OUTCOME_RE = re.compile(
    r"(?m)^(?:outcome\s+definition|endpoint\s+definition|primary\s+outcome|"
    r"outcomes?)\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\b(?:outcomes?\s+were|overall\s+outcome|secondary\s+analysis|result|"
    r"positive\s+outcome)\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:primary\s+)?(?:outcome|endpoint)\s*(?:was\s+defined\s+as|:)\s+"
    r"[^\.\n]{0,100}",
    re.I,
)
...
def find_outcome_definition_v3(text: str, block_chars: int = 400):
    ...
    for h in HEADING_OUTCOME_RE.finditer(text):
        ...
        nb = text.find("\n\n", start)
            ...
            nb
            if 0 <= nb - start <= block_chars
            else min(len(text), start + block_chars)
        )
        block_text = text[start:end].strip()
        if (
            block_text
            and not TRAP_RE.search(block_text)
            and not re.fullmatch(
                r"(not specified\.?|\(no outcome reported\))", block_text, re.I
            )
        ):
...
def find_outcome_definition_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snippet in matches:
        ...
        if CRITERION_TOKEN_RE.search(window_text) and not re.search(
            r"\bvague\b", window_text, re.I
        ):
            out.append((w_s, w_e, snippet))
    return out


def find_outcome_definition_v5(text: str):
    return _collect([TIGHT_TEMPLATE_RE], text)


OUTCOME_DEFINITION_FINDERS: dict[
    str, Callable[[str], list[tuple[int, int, str]]]
] = {
    "v1": find_outcome_definition_v1,
    "v2": find_outcome_definition_v2,
    "v3": find_outcome_definition_v3,
    "v4": find_outcome_definition_v4,
    "v5": find_outcome_definition_v5,
}
__all__ = [
