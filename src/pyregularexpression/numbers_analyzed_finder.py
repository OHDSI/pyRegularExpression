"""
numbers_analyzed_finder.py – p/r ladder for *numbers analysed*.

Five variants (v1–v5):
    • v1 – high recall: any analysis‑count cue (participants analysed,
      analysed, included in analysis, n = …) plus a number.
    • v2 – v1 **and** explicit group keyword (treatment, control, placebo,
      intervention, arm, group) within ±4 tokens.
    • v3 – only inside a *Numbers Analysed / Analysis Population* heading block
      (first ~400 characters).
    • v4 – v2 plus analysis‑set label (intention‑to‑treat, ITT, per‑protocol,
      PP, safety set) in the same sentence.
    • v5 – tight template: “98 intervention and 102 control participants
      analysed (ITT).”
...
ANALYZE_CUE_RE = re.compile(
    r"\b(?:analys(?:ed|is)|included\s+in\s+analysis|participants?\s+analys(?:"
    r"ed|is)|evaluated|assessed)\b",
    re.I,
)
N_EQUALS_RE = re.compile(r"n\s*=\s*\d+", re.I)
GROUP_RE = re.compile(
    r"\b(?:treatment|intervention|placebo|control|arm|group|cohort)\b", re.I
)
POP_RE = re.compile(
    r"\b(?:intention[- ]to[- ]treat|itt|per[- ]protocol|pp|safety\s+set)\b", re.I
)
HEAD_COUNT_RE = re.compile(
    r"(?m)^(?:numbers?\s+analys(?:ed|ed)|analysis\s+population|participants?\s+"
    r"analys(?:ed|is))\s*[:\-]?\s*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    rf"{NUM_RE}\s+[^ ,;]+\s+and\s+{NUM_RE}\s+[^ ,;]+\s+participants?\s+analys"
    r"(?:ed|is).*?(?:itt|intention[- ]to[- ]treat)",
    re.I,
)
...
def find_numbers_analyzed_v1(text: str):
    pattern = re.compile(
        rf"(?:{ANALYZE_CUE_RE.pattern}|{N_EQUALS_RE.pattern})"
        r"(?:[^\n]{0,15}"
        rf"{NUM_RE})?",
        re.I,
    )
