"""
dose_response_analysis_finder.py – p/r ladder for dose‑response analyses.

Five variants (v1–v5):
    • v1 – high recall: sentence containing a dose‑response cue
      ("dose‑response", dose–effect, exposure‑response, trend test, p‑trend,
      per‑increment, log‑linear, restricted cubic spline).
    • v2 – v1 **and** analytic verb (observed, tested, assessed, evaluated,
      fitted, modelled) within ±4 tokens of the cue.
    • v3 – only inside a *Dose‑Response / Exposure‑Response / Trend Analysis*
      heading block (first ≈400 characters).
    • v4 – v2 plus explicit trend keyword (p‑trend, trend test, spline,
      log‑linear, per 10 mg) in the same sentence.
    • v5 – tight template: “A clear dose‑response was observed (p‑trend <
      0.001).”
...
DOSE_CUE_RE = re.compile(
    r"\b(?:dose[- ]?response|dose[- ]?effect|exposure[- ]?response|e[- ]?r\s+"
    r"relationship|trend\s+test|log[- ]linear|restricted\s+cubic\s+spline|"
    r"p[- ]?trend|per[- ]\d+\s*[a-zA-Z]*|per[- ]increment)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:observed|showed|tested|assessed|evaluated|fitted|fit|model(?:led)?|"
    r"examined|analysed|analyzed)\b",
    re.I,
)
TREND_KEY_RE = re.compile(
    r"\b(?:p[- ]?trend|trend\s+test|log[- ]linear|spline|restricted\s+cubic\s+"
    r"spline)\b",
    re.I,
)
HEAD_DR_RE = re.compile(
    r"(?m)^(?:dose[- ]?response|exposure[- ]?response|trend\s+analysis|"
    r"dose[- ]?effect)\s*[:\-]?\s*$",
    re.I | re.UNICODE,
)
...
TRAP_RE = re.compile(
    r"\breceived\s+\d+\s+doses?|two\s+possible\s+doses|different\s+dose\s+"
    r"groups\s+were\s+assigned\b",
    re.I,
)
...
def find_dose_response_analysis_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_DR_RE.finditer(text)
    ]
...
DOSE_RESPONSE_ANALYSIS_FINDERS: dict[
...
__all__ = [
    ...
    "DOSE_RESPONSE_ANALYSIS_FINDERS",
]
