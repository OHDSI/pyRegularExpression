"""
participant_flow_finder.py – p/r ladder for *participant flow* statements.

Five variants (v1–v5):
    • v1 – high recall: any flow cue (randomized to, allocated to, completed
      the study, n = xx, flow diagram) with a number.
    • v2 – v1 **and** explicit group label or stage keyword (treatment,
      placebo, intervention, control, follow‑up, analysis) within ±4 tokens.
    • v3 – only inside a *Participant Flow* / *CONSORT Flow* / *Figure 1*
      heading block (first ~600 characters).
    • v4 – v2 plus at least two numeric group counts in same sentence/line
      (captures split counts 100 vs 100).
    • v5 – tight template: “200 randomized (100 treatment, 100 placebo); 180
      completed follow‑up.”
...
NUM_RE = r"\d{1,4}"
FLOW_CUE_RE = re.compile(
    r"\b(?:randomi[sz]ed|allocated|assigned|completed|analysed|lost\s+to\s+"
    r"follow[- ]up|withdrew|excluded|screened)\b",
    re.I,
)
GROUP_RE = re.compile(
    r"\b(?:treatment|intervention|placebo|control|drug\s+\w+|arm|group|cohort)\b",
    re.I,
)
STAGE_RE = re.compile(
    r"\b(?:enrol(?:led|ment)|follow[- ]up|analysis|baseline|screening|"
    r"randomi[sz]ation)\b",
    re.I,
)
HEADING_FLOW_RE = re.compile(
    r"(?m)^(?:participant\s+flow|consort\s+flow|figure\s+1)\s*[:\-]?\s*$", re.I
)
TRAP_RE = re.compile(r"\btotal\s+of\s+\d+\b", re.I)
TIGHT_TEMPLATE_RE = re.compile(
    rf"{NUM_RE}\s+randomi[sz]ed\s*\(\s*{NUM_RE}\s+[^,]+,\s*{NUM_RE}\s+[^\)]+\)"
    rf"\s*;\s*{NUM_RE}\s+completed",
    re.I,
)
...
def find_participant_flow_v2(text: str, window: int = 4):
    ...
    grp_idx = {
        ...
        if GROUP_RE.fullmatch(t) or STAGE_RE.fullmatch(t)
    }
    out = []
    for c in cue_idx:
...
def find_participant_flow_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snip in matches:
        nums_near = sum(
            1
            for i in range(max(0, w_s - window), min(len(tokens), w_e + window))
            if NUM_TOKEN_RE.fullmatch(tokens[i])
        )
        if nums_near >= 2:
