"""competing_risk_analysis_finder.py – precision/recall ladder for
*competing‑risk analyses*.

Five variants (v1–v5):
    • v1 – high recall: any sentence containing a competing‑risk cue ("competing
           risk", Fine‑Gray, sub‑hazard ratio, sHR, cumulative incidence
           competing risk).
    • v2 – v1 **and** a modelling verb (fitted, estimated, modelled, applied, used)
           within ±4 tokens of the cue.
    • v3 – only inside a *Competing Risk / Fine‑Gray / Cumulative Incidence*
           heading block (first ≈400 characters).
    • v4 – v2 plus an explicit technique keyword (Fine‑Gray, sub‑hazard,
           cumulative incidence function, sHR) in the same sentence.
    • v5 – tight template: “We fitted Fine‑Gray models to estimate sHRs for
           death vs transplant.”
...
CR_CUE_RE = re.compile(
    r"\b(?:competing\s+risk(?:s)?|fine[–-]?gray|sub[- ]?hazard\s+ratio|shr|subhazard"
    r"|cumulative\s+incidence\s+competing\s+risk)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:fitted|fit|estimated|model(?:led)?|applied|used|performed)\b", re.I
)
TECH_RE = re.compile(
    r"\b(?:fine[–-]?gray|sub[- ]?hazard|cumulative\s+incidence\s+function|shr"
    r"|cause[- ]specific)\b",
    re.I,
)
HEAD_CR_RE = re.compile(
    r"(?m)^(?:competing\s+risk(?:s)?|fine[–-]?gray|cumulative\s+incidence)\s*[:\-]?\s*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"fitted\s+fine[–-]?gray\s+model[s]?[^\.\n]{0,40}shr", re.I
)
TRAP_RE = re.compile(r"\bcompetition\s+for\s+resources|risk\s+competition\b", re.I)
NEG_RE = re.compile(
    r"\b(?:without|not|no|absence(?:\s+of)?|lacking|lack|did\s+not|didn’t|didn't"
    r"|never|rather\s+than)\b",
    re.I,
)
...
def find_competing_risk_analysis_v2(text: str, window: int = 4):
    ...
    for c_s, c_e in cue_idx:
        if any(
            abs(v_s - c_s) <= window or abs(v_e - c_e) <= window
            for v_s, v_e in verb_idx
        ):
...
def find_competing_risk_analysis_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_CR_RE.finditer(text)
    ]
...
COMPETING_RISK_ANALYSIS_FINDERS: dict[
...
__all__ = [
    ...
    "COMPETING_RISK_ANALYSIS_FINDERS",
]
