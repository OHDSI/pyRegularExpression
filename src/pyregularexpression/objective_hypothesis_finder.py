"""
objective_hypothesis_finder.py – p/r ladder for *study objectives*.

Five variants (v1–v5):
    • v1 – high recall: any objective/aim/hypothesis cue (e.g., “the aim of
      this study”, “we aimed to”, “our objective was”, “we hypothesize that”,
      “to determine whether”).
    • v2 – objective/aim cue + verb tense (was/were/aimed) within ±3 tokens
      **or** hypothesis phrase (“we hypothesize/hypothesized that”).
    • v3 – only inside an *Objectives / Aims / Purpose* heading block.
    • v4 – v2 plus explicit study reference token (“this study”, “the study”)
      to avoid generic uses like “objective measurement”.
    • v5 – tight template: “The objective of this study was to assess…”, “We
      hypothesized that exposure X increases risk Y…”.
...
OBJ_CUE_RE = re.compile(
    r"\b(?:the\s+aim\s+of\s+this\s+study|we\s+aim(?:ed)?\s+to|our\s+objective\s+"
    r"was|the\s+objective\s+of\s+this\s+study|this\s+study\s+seeks\s+to|purpose"
    r"\s+of\s+this\s+study|to\s+determine\s+whether)\b",
    re.I | re.UNICODE,
)
HYP_CUE_RE = re.compile(r"\bwe\s+hypothes(?:is|iz)(?:e|ed)?\s+that\b", re.I)
STUDY_TOKEN_RE = re.compile(r"\b(?:study|this study)\b", re.I)
HEADING_OBJ_RE = re.compile(
    r"(?m)^(?:objectives?|aims?|purpose|study\s+aims?)\s*[:\-]?\s*$", re.I
)
TRAP_RE = re.compile(
    r"\bobjective\s+(?:measurement|value)|aim\s+for|objective\s+function\b", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:(?:the\s+objective\s+of\s+this\s+study\s+was\s+to)|(?:we\s+aim(?:ed)?\s+"
    r"to\s+(?!study\b))|(?:we\s+hypothes(?:is|iz)(?:e|ed)?\s+that))",
    re.I | re.DOTALL,
)
...
def find_objective_hypothesis_v2(text: str, window: int = 3):
    ...
    verb_re = re.compile(
        r"\b(?:was|were|is|are|aim(?:ed)?|hypothes(?:is|iz)(?:e|ed)?)\b", re.I
    )
...
# Mapping & exports
# ─────────────────────────────
OBJECTIVE_HYPOTHESIS_FINDERS: dict[
