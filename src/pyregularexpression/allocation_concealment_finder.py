"""
allocation_concealment_finder.py – precision/recall ladder for *allocation
concealment* methods.

Five variants (v1–v5):

- v1 – high recall: any concealment cue (opaque sealed envelopes, central
  randomization, allocation concealment phrase, assignments sealed).
- v2 – concealment cue + randomisation keyword (allocation, sequence, randomized)
  within ±4 tokens.
- v3 – within an *Allocation Concealment / Randomisation* heading block.
- v4 – v2 plus explicit descriptor (central, telephone, web-based,
  pharmacy-controlled, sequentially numbered) nearby.
- v5 – tight template: “Assignments in sequentially numbered opaque envelopes
  ensured concealment.”
...
CONCEAL_CUE_RE = re.compile(
    r"\b(?:opaque\s+sealed\s+envelopes?|sealed\s+opaque\s+envelopes?|"
    r"sequentially\s+numbered\s+opaque\s+envelopes?|central(?:ised|ized)?\s+"
    r"randomi[sz]ation|central\s+allocation|telephone\s+randomi[sz]ation|web"
    r"[- ]?based\s+randomi[sz]ation|pharmacy[- ]?controlled|allocation\s+"
    r"concealment)\b",
    re.I,
)
RAND_KEY_RE = re.compile(
    r"\b(?:allocation|sequence|randomi[sz]ed|randomi[sz]ation)\b", re.I
)
DESC_RE = re.compile(
    r"\b(?:central(?:ised|ized)?|telephone|web[- ]?based|pharmacy[- ]?"
    r"controlled|sequentially|numbered)\b",
    re.I,
)
HEADING_CONC_RE = re.compile(
    r"(?m)^(?:allocation\s+concealment|concealment|randomi[sz]ation)\s*[:\-]?"
    r"\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\bconcealed\s+allocation\s+was\s+not\s+possible|blinded\s+assessors\b", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"assignments?\s+in\s+sequentially\s+numbered\s+opaque\s+envelopes?\s+"
    r"(?:ensured|achieved)\s+allocation\s+concealment",
    re.I,
)
...
def find_allocation_concealment_v2(text: str, window: int = 6):
    ...
    for m in conc_matches:
        w_s, w_e = _char_to_word((m.start(), m.end()), spans)
        # Check if any randomization keyword is within ±window tokens of the
        # concealment cue
...
ALLOCATION_CONCEALMENT_FINDERS: dict[
...
__all__ = [
    ...
    "ALLOCATION_CONCEALMENT_FINDERS",
]
