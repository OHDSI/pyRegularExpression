...
def _char_span_to_word_span(
    ...
    """Convert a character slice to the **inclusive** word‑index span."""
...
CODE_TERM = (
    ...
    r"international\sclassification\s+of\s+diseases(?:[- ]?(?:9|10|11))?)|"
    r"cpt|current\s+procedural\s+terminology(?:[- ]?4)?|hcpcs|healthcare\s+"
    r"common\s+procedure\s+coding\s+system|snomed(?:[ -]?ct)?|rxnorm|loinc|"
    r"read\s+codes?|icpc|atc(?:\s+codes?)?"
)
...
GENERIC_CODE_TRAP_RE = re.compile(
    r"\b(?:programming|source|survey|genetic|study[- ]?id)\s+codes?\b", re.I
)
NEEDS_ACRONYM_RE = re.compile(
    r"\b(?:icd[- ]?\d{1,2}|cpt|snomed(?:\s+ct)?|rxnorm|loinc|diagnosis|"
    r"procedure|billing|medical|clinical)\b",
    re.I,
)

NEGATIVE_PRE_TOKEN_RE = re.compile(r"^(?:#|mg|kg|mmhg|bpm)$", re.I)
HEADING_RE = re.compile(
    r"(?m)^(?:diagnosis|procedure|billing|icd-?\d{1,2}?|codes?)\s+codes?:\s*$",
    re.I,
)
...
# ─────────────────────────────
def _is_short_numeric_false_positive(
    match: re.Match[str], text: str, min_len: int = 4
) -> bool:
    token = match.group(0)
...
def find_medical_code_v5(text: str):  # strict stand‑alone
    ...
    return [
        ...
        if STRICT_TOKEN_RE.fullmatch(tok)
    ]
