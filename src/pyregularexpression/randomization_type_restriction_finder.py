"""
randomization_type_restriction_finder.py – p/r ladder for *randomization*.

Five variants (v1–v5):
    • v1 – high recall: any restriction cue (block randomization, permuted
      blocks, stratified by, minimization, 1:1 ratio, 2:1 ratio).
    • v2 – restriction cue + randomisation keyword (randomized, allocation,
      sequence) within ±4 tokens.
    • v3 – only inside a *Randomisation / Allocation* heading block (first
      ~400 characters).
    • v4 – v2 plus explicit allocation ratio (e.g., 1:1, 2:1) or multiple
      modifiers (block + stratified) in same sentence.
    • v5 – tight template: “Randomized 2:1 to drug vs placebo using permuted
      blocks of six, stratified by site.”
...
def _char_to_word(span: tuple[int, int], spans: Sequence[tuple[int, int]]):
    s, e = span
    w_s = next(
        i for i, (a, b) in enumerate(spans) if a <= s < b or (s <= a and s < b)
    )
    w_e = next(
        i
...
RESTRICT_CUE_RE = re.compile(
    r"\b(?:block\s+randomi[sz]ation|permuted\s+blocks?|minimi[sz]ation|"
    r"stratified\s*(?:by)?|strata|shuffled\s+envelopes)\b",
    re.I,
)
RATIO_RE = re.compile(r"\b\d+:\d+\b")
RAND_KEY_RE = re.compile(
    r"\b(?:randomi[sz](?:ed|ation)|allocation|sequence|assigned)\b", re.I
)
MODIFIER_RE = re.compile(
    r"\b(?:block|blocks?|permuted|stratified|minimization|strata|ratio)\b", re.I
)
HEADING_RAND_RE = re.compile(
    r"(?m)^(?:randomi[sz]ation|allocation|sequence\s+generation)\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\brandomly\s+assigned|random\s+sampling|random\s+effects?\b", re.I
)
...
def find_randomization_type_restriction_v4(text: str, window: int = 6):
    """v2 plus explicit allocation ratio OR multiple modifiers."""
    spans = _token_spans(text)
    ...
    for w_s, w_e, snip in matches:
        mods_near = sum(
            1 for m in mod_idx if w_s - window <= m <= w_e + window
        )
...
RANDOMIZATION_TYPE_RESTRICTION_FINDERS: dict[
...
__all__ = [
    ...
    "RANDOMIZATION_TYPE_RESTRICTION_FINDERS",
]

find_randomization_type_restriction_high_recall = (
    find_randomization_type_restriction_v1
)
find_randomization_type_restriction_high_precision = (
    find_randomization_type_restriction_v5
)
