"""
algorithm_validation_finder.py – precision/recall ladder for *algorithm validation*
statements.

Five variants (v1–v5):
    • v1 – high recall: any phrase “algorithm validation/validated/evaluated” or
           performance metric keywords.
    • v2 – algorithm keyword + validation verb (validated/evaluated/assessed) within
           ±window tokens.
    • v3 – only inside an *Algorithm validation / Performance evaluation* heading block.
    • v4 – v2 plus metric tokens (PPV, NPV, sensitivity, specificity, accuracy, F1,
           AUC) to exclude algorithm-use-only context.
    • v5 – tight template: “Algorithm validated against chart review; PPV 92 %”,
           “Accuracy 0.89 (95 % CI) in external validation”, etc.

Each finder returns a list of tuples:
...
def _char_span_to_word_span(
    ...
    w_end = next(
        i for i, (s, e) in reversed(list(enumerate(token_spans))) if s < e_char <= e
    )
    return w_start, w_end


ALGO_TERM_RE = re.compile(r"\balgorithm\b", re.I)
VALIDATE_VERB_RE = re.compile(
    r"\b(?:validated|validation|evaluated|assessed|tested|performance)\b",
    re.I,
)
METRIC_TOKEN_RE = re.compile(
    r"\b(?:ppv|npv|positive\s+predictive\s+value|negative\s+predictive\s+value"
    r"|sensitivity|specificity|accuracy|f1|auc|area\s+under\s+the\s+curve|kappa)\b",
    re.I,
)
HEADING_VALID_RE = re.compile(
    r"(?m)^(?:algorithm\s+validation|validation\s+study|performance\s+evaluation)"
    r"\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\b(?:validated\s+questionnaire|assay\s+validation|method\s+validation)\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"algorithm\s+(?:was\s+)?(?:validated|evaluated|assessed)[^\.\n]{0,80}"
    r"(?:ppv|accuracy|sensitivity|specificity|auc|f1)\b",
    re.I,
)
...
def find_algorithm_validation_v1(text: str):
    """
    Find algorithm validation statements using a high-recall approach.
    """
    return _collect(
        [
            re.compile(r"algorithm\s+validation", re.I),
            VALIDATE_VERB_RE,
            METRIC_TOKEN_RE,
        ],
        text,
...
def find_algorithm_validation_v2(text: str, window: int = 4):
    """
    Find algorithm validation statements with validation verbs within a window.
    """
    token_spans = _token_spans(text)
...
def find_algorithm_validation_v3(text: str, block_chars: int = 300):
    """
    Find algorithm validation statements within specific heading blocks.
    """
    token_spans = _token_spans(text)
    blocks = []
...
def find_algorithm_validation_v4(text: str, window: int = 6):
    """
    Find algorithm validation statements with metric tokens within a window.
    """
    token_spans = _token_spans(text)
    tokens = [text[s:e] for s, e in token_spans]
...
def find_algorithm_validation_v5(text: str):
    """
    Find algorithm validation statements using a tight template.
    """
