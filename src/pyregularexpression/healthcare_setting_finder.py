"""
healthcare_setting_finder.py – p/r ladder for *health‑care setting*.

Five variants (v1–v5):
    • v1 – high recall: any facility/setting term (inpatient, outpatient,
      ICU, etc.).
    • v2 – facility term within ±3 tokens of a context word (setting, clinic,
      care, unit, hospital).
    • v3 – only inside a *Setting / Healthcare setting* heading block.
    • v4 – v2 plus qualifier tokens (primary, secondary, tertiary, academic,
      community) to filter generic mentions.
    • v5 – tight template: “Conducted in five primary‑care clinics”, “Data from
      an ICU inpatient setting”, etc.
...
def normalize_text(text: str) -> str:
    """Normalize the text and replace non-breaking hyphens."""
    # Normalize the text to NFC form, which handles non-breaking hyphens
    text = unicodedata.normalize("NFC", text)
    return text.replace("\u2011", "-")  # Replace non-breaking hyphen
...
FACILITY_RE = re.compile(
    r"\b(hospitals?|medical centers?|healthcare centers?|(?:outpatient|"
    r"primary care|community|specialty)\s+clinics?|ICU(?:s| wards?)?|intensive"
    r" care units?|ICU wards?|wards?|pharmacies|pharmacy|community pharmacy|"
    r"inpatients?|outpatients?)\b",
    re.I,
)

CONTEXT_RE = re.compile(
    r"^(?:setting|settings|clinic|care|unit|environment|data|patients?|ward|"
    r"healthcare|facility|medical|hospitalization|treatment|caregiver)[\.,;:]?$",
    re.I,
)

QUALIFIER_RE = re.compile(
    r"\b(?:primary|secondary|tertiary|academic|community|teaching|urban|rural|"
    r"outpatient|ambulatory|regional|suburban|specialist|private|public)\b",
    re.I,
)

HEADING_SET_RE = re.compile(
    r"(?m)^(?:setting|healthcare\s+setting|study\s+setting|study\s+design|"
    r"research\s+setting|care\s+setting|clinical\s+setting|service\s+setting)"
    r"\s*[:\-]?\s*.*$",
    re.I,
)
...
TIGHT_TEMPLATE_RE = re.compile(
    ...
    r"[^.\n]{0,80}?(?:inpatient\s+setting|outpatient\s+setting|primary[-\s]+"
    r"care\s+clinics?|icu(?:\s+inpatient\s+setting)?|hospital(?:\s+ward)?)\b",
    re.I,
)
...
def find_healthcare_setting_v3(text: str):
    ...
    for match in re.finditer(
        r"(healthcare setting|study setting)\s*:\s*(.*?)(?:\n\s*\n|$)",
        text_norm,
        re.I | re.S,
    ):
...
# 4. Mapping & exports
# ─────────────────────────────
HEALTHCARE_SETTING_FINDERS: dict[
