"""
outcome_ascertainment_finder.py – p/r ladder for *outcome ascertainment*.

Five variants (v1–v5):
    • v1 – high recall: any ascertainment verb cue (ascertained/identified/
      confirmed/etc.)
    • v2 – cue + source/preposition (via/through/from/using) within ±window
      tokens
    • v3 – only inside an *Outcome ascertainment / Event verification* heading
      block
    • v4 – v2 plus explicit outcome/event keyword (stroke, death, MI, cancer,
      etc.) or imaging/chart review term, excludes bias-only mentions
    • v5 – tight template: “Stroke events verified via imaging”, “Outcomes were
      confirmed by medical record review”, etc.
...
SOURCE_PREP_RE = re.compile(
    r"\b(?:via|through|from|using|with|based\s+on|by)\b", re.I
)

OUTCOME_KEYWORD_RE = re.compile(
    r"\b(?:stroke|death|mortality|myocardial\s+infarction|mi|cancer|"
    r"hospitalisation|event|outcome|endpoint)\b",
    re.I,
)

DATASET_TERM_RE = re.compile(
    r"\b(?:medical\s+records?|chart\s+review|claims?|imaging|registry|database|"
    r"ehr|electronic\s+health\s+records?)\b",
    re.I,
)

HEADING_ASCERT_RE = re.compile(
    r"(?m)^(?:outcome\s+ascertainment|event\s+ascertainment|event\s+"
    r"verification|event\s+adjudication)\s*[:\-]?\s*$",
    re.I,
)
...
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:events?|outcomes?)\s+(?:were\s+)?(?:ascertained|confirmed|verified|"
    r"identified)\s+(?:via|through|from|using|by)\s+[^\.\n]{0,80}",
    re.I,
)
...
def find_outcome_ascertainment_v3(text: str, block_chars: int = 400):
    ...
    for h in HEADING_ASCERT_RE.finditer(text):
        s = h.end()
        nb = text.find("\n\n", s)
        e = nb if 0 <= nb - s <= block_chars else s + block_chars
        blocks.append((s, e))
...
def find_outcome_ascertainment_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snippet in matches:
        ...
        if OUTCOME_KEYWORD_RE.search(window_text) or DATASET_TERM_RE.search(
            window_text
        ):
...
OUTCOME_ASCERTAINMENT_FINDERS: dict[
    str, Callable[[str], list[tuple[int, int, str]]]
] = {
    "v1": find_outcome_ascertainment_v1,
    "v2": find_outcome_ascertainment_v2,
    "v3": find_outcome_ascertainment_v3,
    "v4": find_outcome_ascertainment_v4,
    "v5": find_outcome_ascertainment_v5,
}
__all__ = [
