"""comparator_cohort_finder.py – precision/recall ladder for
*comparator (control) cohort* statements.
...
Five variants (v1–v5):
    ...
    • v4 – v2 plus explicit qualifier (unexposed, matched, reference) excluding
           generic comparisons
    • v5 – tight template: “Matched unexposed cohort served as comparator”,
           “Control group comprised patients receiving placebo”, etc.
...
COMP_KEYWORD_RE = re.compile(
    r"""
    \b(
        (?:control|comparator|comparison|placebo|reference)
        (?:\s+(?:group|arm|cohort|subjects))?|
        (?:intervention\s+and\s+control|control\s+and\s+intervention)\s+groups?
    )\b
    """,
    re.I | re.VERBOSE,
)
...
QUALIFIER_RE = re.compile(
    r"\b(?:unexposed|matched|reference|placebo|standard\s+care)\b", re.I
)

HEADING_COMP_RE = re.compile(
    r"(?m)^(?:control\s+cohort|control\s+group|comparator\s+group|comparison\s+group"
    r"|reference\s+cohort)\s*[:\-]?\s*$",
    re.I,
)

DIVIDED_GROUPS_RE = re.compile(
    r"\bdivided\s+into\s+(intervention\s+and\s+control|control\s+and\s+intervention)"
    r"\s+groups\b",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:compared\s+to|comparison\s+with|device\s+comparator|comparative\s+analysis)\b",
    re.I,
)
...
TIGHT_TEMPLATE_RE = re.compile(
    r"""(?x)
        (?:
            ...
            (?:control\s+group)\s+comprised\s+patients\s+receiving\s+
                (?:placebo|standard\s+care|usual\s+care)
        )
    """,
    re.I,
)
...
def find_comparator_cohort_v3(
    ...
    for heading_match in HEADING_COMP_RE.finditer(text):
        ...
        matches.append(
            (heading_match.start(), heading_match.end(), heading_match.group())
        )
    return matches


def find_comparator_cohort_v4(
