"""
propensity_score_method_finder.py – p/r ladder for *propensity-score*.

Five variants (v1–v5):
    • v1 – high recall: any sentence with a propensity‑score cue (propensity
      score, PS-matched, IPTW, inverse probability weighting, doubly robust).
    • v2 – v1 **and** analysis verb (calculated, estimated, applied, used,
      performed) within ±4 tokens.
    • v3 – only inside a *Propensity Score / Confounding Control* heading
      block (first ~400 characters).
    • v4 – v2 plus explicit technique qualifier (matching, weighting,
      stratification, IPTW, SMR, stabilized) in the same sentence.
    • v5 – tight template: “We estimated propensity scores via logistic
      regression and applied IPTW.”
...
PS_CUE_RE = re.compile(
    r"\b(?:propensity\s+scores?|ps[- ]?matched|ps[- ]?weight(?:ed|ing)?|iptw|"
    r"ipw|smr\s+weight(?:ed|ing)?|inverse\s+probability\s+weight(?:ed|ing)?|"
    r"doubly\s+robust|augmented\s+iptw)\b",
    re.I,
)
VERB_RE = re.compile(
    r"\b(?:calculated|estimated|computed|derived|applied|used|performed|"
    r"implemented)\b",
    re.I,
)
TECH_RE = re.compile(
    r"\b(?:matching|weighting|stratification|iptw|inverse\s+probability|smr|"
    r"stabilized|fine\s+stratification|doubly\s+robust)\b",
    re.I,
)
HEAD_PS_RE = re.compile(
    r"(?m)^(?:propensity\s+score|confounding\s+control|ps\s+method)\s*[:\-]?\s*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"estimated\s+propensity\s+scores?[^\.\n]{0,40}?applied\s+(?:iptw|"
    r"ps[- ]?matching|inverse\s+probability\s+weight(?:ed|ing))",
    re.I,
)
...
PROPENSITY_SCORE_METHOD_FINDERS: dict[
...
__all__ = [
    ...
    "PROPENSITY_SCORE_METHOD_FINDERS",
]
