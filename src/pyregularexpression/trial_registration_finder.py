"""
trial_registration_finder.py – p/r ladder for *trial registration*.

Five variants (v1–v5):
    • v1 – high recall: any sentence with a registration cue ("trial
      registration", registered at ClinicalTrials.gov, NCT########, ISRCTN,
      EudraCT, ChiCTR).
    • v2 – v1 **and** registration verb (registered, prospectively
      registered, recorded) within ±4 tokens of the cue or identifier.
    • v3 – only inside a *Trial Registration* or *Registration* heading block
      (first ≈400 characters).
    • v4 – v2 plus explicit registry identifier pattern (NCT\\d{8},
      ISRCTN\\d+, EudraCT \\d{4}-\\d{6}-\\d{2}, ChiCTR-\\w+).
    • v5 – tight template: “This trial was prospectively registered at
...
REGISTRY_ID_RE = re.compile(
    r"\b(?:NCT\d{8}|ISRCTN\d{6,8}|EudraCT\s*\d{4}-\d{6}-\d{2}|ChiCTR(?:-[\w\d]+)?"
    r"|ACTRN\d{14}|JPRN-UMIN\d{9}|ClinicalTrials\.gov|ISRCTN|EudraCT|ChiCTR|"
    r"ANZCTR|JPRN)\b",
    re.I,
)
REG_CUE_PATTERNS = [
...
VERB_RE = re.compile(
    r"\b(?:registered|recorded|submitted|prospectively\s+registered)\b", re.I
)
HEAD_REG_RE = re.compile(
    r"(?m)^(?:trial\s+registration|registration)\s*[:\-]?\s*$", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"((?:this\s+)?trial\s+was\s+prospectively\s+registered(?:\s+at\s+[\w\.]+)?\s*"
    r"\(?(?:NCT\d{8}|ISRCTN\d{6,8}|EudraCT\s*\d{4}-\d{6}-\d{2}|ChiCTR(?:-[\w\d]+)?)\)?)",
    re.I,
)
...
def find_trial_registration_v1(text: str) -> list[tuple[int, int, str]]:
    """Tier 1 – high recall: any registration cue or registry ID."""
    spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
...
def find_trial_registration_v3(text: str, block_chars: int = 400):
    ...
    blocks = [
        ...
        for h in HEAD_REG_RE.finditer(text)
    ]
...
def find_trial_registration_v4(text: str, window: int = 6):
    ...
    id_idx = {
        i for i, t in enumerate(tokens) if REGISTRY_ID_RE.fullmatch(t.strip(".,"))
    }
...
def find_trial_registration_v5(text: str) -> list[tuple[int, int, str]]:
    """Tier 5 – tight template: prospectively registered trial with ID."""
    return _collect([TIGHT_TEMPLATE_RE], text)


TRIAL_REGISTRATION_FINDERS: dict[
