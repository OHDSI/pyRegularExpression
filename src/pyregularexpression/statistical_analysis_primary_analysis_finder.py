"""
statistical_analysis_primary_analysis_finder.py – p/r ladder for *primary analysis*.

Five variants (v1–v5):
    • v1 – high recall: lines/clauses containing *primary outcome/endpoint*
      plus an analysis verb (analysed, assessed, modelled, evaluated) **or** an
      ITT/per‑protocol phrase.
    • v2 – v1 **and** explicit statistical test/model keyword (Cox model,
      mixed‑effects, logistic regression, Kaplan–Meier, t‑test, chi‑square,
      etc.) within ±4 tokens.
    • v3 – only inside a *Statistical Analysis* or *Primary Analysis* heading
      block (first ~500 characters).
    • v4 – v2 plus adjustment phrase (adjusting for, covariates, baseline) or
      modelling details (random effects, fixed effects, repeated measures) in
      the same sentence.
    • v5 – tight template: “Primary endpoint analysed with mixed‑effects linear
      model adjusting for baseline covariates.”
...
ANALYSIS_VERB_RE = re.compile(
    r"\b(?:analys(?:ed|is)|model(?:ed|ling)?|assess(?:ed|ment)?|evaluat(?:ed|ion)"
    r"|tested)\b",
    re.I,
)
ITT_RE = re.compile(
    r"\b(?:intention[- ]to[- ]treat|per[- ]protocol|modified\s+itt|mITT)\b", re.I
)
STAT_TEST_RE = re.compile(
    r"\b(?:cox(?:\s+proportional\s+hazards)?|kaplan[- ]meier|log[- ]rank|mixed[- ]"
    r"effects?|generalised\s+estimating\s+equations|gee|linear\s+mixed|"
    r"logistic\s+regression|poisson\s+regression|negative\s+binomial|anova|"
    r"t[- ]test|chi[- ]square|fisher'?s\s+exact|wilcoxon|mann[- ]whitney|"
    r"hazard\s+ratio|rate\s+ratio)\b",
    re.I,
)
ADJUST_RE = re.compile(
    r"\b(?:adjust(?:ed|ing)?\s+for|covariate|baseline|stratified\s+by|random\s+"
    r"effects|fixed\s+effects|repeated\s+measures)\b",
    re.I,
)
HEAD_STAT_RE = re.compile(
    r"(?m)^(?:statistical\s+analysis(?:es)?|analysis|primary\s+analysis)\s*"
    r"[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(r"\bp\s*<\s*0\.\d+|significant|confidence\s+interval\b", re.I)
TIGHT_TEMPLATE_RE = re.compile(
    r"primary\s+(?:endpoint|outcome)\s+analysed\s+with\s+mixed[- ]effects?\s+"
    r"[^\".\n]{0,40}?adjust(?:ed|ing)\s+for\b",
    re.I,
)
...
def find_statistical_analysis_primary_analysis_v1(text: str):
    patterns = [
        re.compile(
            rf"{PRIMARY_KEY_RE.pattern}[^\.\n]{{0,10}}{ANALYSIS_VERB_RE.pattern}",
            re.I,
        ),
        ITT_RE,
    ]
...
def find_statistical_analysis_primary_analysis_v2(text: str, window: int = 4):
    spans = _token_spans(text)
    [text[s:e].strip(".,") for s, e in spans]
    prim_idx = []
...
def find_statistical_analysis_primary_analysis_v3(
...
STATISTICAL_ANALYSIS_PRIMARY_ANALYSIS_FINDERS: dict[
...
__all__ = [
    ...
    "STATISTICAL_ANALYSIS_PRIMARY_ANALYSIS_FINDERS",
]

find_statistical_analysis_primary_analysis_high_recall = (
    find_statistical_analysis_primary_analysis_v1
)
find_statistical_analysis_primary_analysis_high_precision = (
    find_statistical_analysis_primary_analysis_v5
)
