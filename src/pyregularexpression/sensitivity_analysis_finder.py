"""
sensitivity_analysis_finder.py – p/r ladder for *sensitivity analysis*.

Five variants (v1–v5):
    • v1 – high recall: any occurrence of “sensitivity analysis/analyses”.
    • v2 – sensitivity phrase + analysis verb (performed/conducted/repeated)
      within ±window tokens.
    • v3 – only inside a *Sensitivity analysis* heading block.
    • v4 – v2 plus scenario/assumption tokens (excluding, removing,
      alternative, varying) to exclude generic mentions.
    • v5 – tight template: “Excluding switchers in sensitivity analysis…”,
      “Sensitivity analyses repeated with alternative cut‑offs…”, etc.
...
ANALYSIS_VERB_RE = re.compile(
    r"\b(?:performed|conducted|repeated|ran|undertook|carried\s+out)\b", re.I
)

SCENARIO_TOKEN_RE = re.compile(
    r"\b(?:excluding|removing|restricting|alternative|varying|assumption|"
    r"switchers|per[- ]?protocol|as[- ]?treated)\b",
    re.I,
)

HEADING_SENS_RE = re.compile(
    r"(?im)^\s*sensitivity\s+analys(?:is|es)\s*[:\-]?\s*$"
)

TRAP_RE = re.compile(r"\bassay\s+sensitivity\b|\bsensitivity\s+\d{1,3}\s*%", re.I)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:sensitivity\s+analys(?:is|es)\s+(?:were\s+)?(?:performed|conducted|"
    r"repeated)|excluding\s+[^\.\n]{0,60}\s+in\s+sensitivity\s+analys"
    r"(?:is|es))",
    re.I,
)
...
def find_sensitivity_analysis_v2(
...
def find_sensitivity_analysis_v3(text: str) -> list[tuple[int, int, str]]:
    """Detect sensitivity analysis phrases only inside heading blocks."""
    token_spans = _token_spans(text)
    out: list[tuple[int, int, str]] = []
...
def find_sensitivity_analysis_v4(
...
# 4.  Mapping & exports
# ─────────────────────────────
SENSITIVITY_ANALYSIS_FINDERS: dict[
