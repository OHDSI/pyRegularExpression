"""
harms_adverse_event_finder.py – p/r ladder for *harms / adverse events*.

Five variants (v1–v5):
    • v1 – high recall: any harms cue (adverse events, side effects,
      complications) followed by a number / percent.
    • v2 – v1 **and** explicit group or comparison keyword (treatment, control,
      placebo, vs, compared to) within ±4 tokens.
    • v3 – only inside a *Harms / Adverse Events* heading block (first ~400
      characters).
    • v4 – v2 plus serious/severity descriptor (serious, severe, grade ≥3) in
      the same sentence.
    • v5 – tight template: “15 % headaches in treatment vs 10 % placebo; no
      serious events.”
...
AE_CUE_RE = re.compile(
    r"\b(?:adverse\s+events?|side\s+effects?|complications?)\b", re.I
)
GROUP_RE = re.compile(
    r"\b(?:treatment|intervention|placebo|control|arm|group|vs|versus|"
    r"compared\s+to)\b",
    re.I,
)
SEVERITY_RE = re.compile(
    r"\b(?:serious|severe|grade\s*[3-5]|grade\s*≥\s*3|no\s+serious)\b", re.I
)
HEAD_AE_RE = re.compile(
    r"(?m)^(?:harms?|adverse\s+events?|safety|tolerability)\s*[:\-]?\s*$", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    rf"{NUM_RE}\s+[^,;\n]+\s+vs\s+{NUM_RE}\s+[^,;\n]+;?\s+no\s+serious\s+events",
    re.I,
)
...
def _collect(patterns: Sequence[re.Pattern[str]], text: str):
    ...
    for patt in patterns:
        for m in patt.finditer(text):
            if TRAP_RE.fullmatch(m.group(0)):
                continue
...
def find_harms_adverse_event_v2(text: str, window: int = 4):
    ...
    for c_start, c_end in cue_idx:
        if any(
            abs(n_start - c_start) <= window or abs(n_end - c_end) <= window
            for n_start, n_end in num_idx
        ) and any(
            abs(g_start - c_start) <= window or abs(g_end - c_end) <= window
            for g_start, g_end in grp_idx
        ):
...
def find_harms_adverse_event_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snip in matches:
        if any(
            abs(w_s - s_start) <= window or abs(w_e - s_end) <= window
            for s_start, s_end in sev_idx
        ):
...
HARMS_ADVERSE_EVENT_FINDERS: dict[
