"""
severity_definition_finder.py – p/r ladder for *severity definition*.

Five variants (v1–v5):
    • v1 – high recall: any severity cue (mild/moderate/severe/severity)
    • v2 – cue + defining verb (defined/classified/categorised) within
      ±window tokens
    • v3 – only inside a *Severity definition / Classification* heading block
    • v4 – v2 plus explicit multi-level listing (mild+moderate+severe) or
      comparison/threshold token (≥, IV antibiotics, admission)
    • v5 – tight template: “Severity was defined by …”, “Disease classified as
      mild/moderate/severe based on …”, etc.
...
SEVERITY_TERM_RE = re.compile(
    r"\b(?:severity|mild|moderate|severe)\b(?!\s+weather\b)", re.I
)
DEFINE_VERB_RE = re.compile(
    r"\b(?:defined|classified|categoris(?:ed|ed)|graded|stratified|assessed)\b",
    re.I,
)
LISTING_PATTERN_RE = re.compile(
    r"mild\s*(?:[\/,]| and )\s*moderate\s*(?:[\/,]| and )\s*severe(?:\s+"
    r"[a-zA-Z ]+)?",
    re.I,
)
THRESHOLD_TOKEN_RE = re.compile(
    r"\b(?:>=|<=|>|<|iv\s+antibiotics|admission|hospitalisation|oxygen|\d+\s*"
    r"points?)\b",
    re.I,
)
HEADING_SEVERITY_RE = re.compile(
    r"(?m)^(?:severity|classification)\s*(?:definition|grading|was recorded)?"
    r"\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(
    r"\b(?:severe|moderate|mild)\b(?![^\.]{0,40}(?:defined|classified))", re.I
)
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:severity\s+(?:was\s+)?defined\s+(?:by|as)\s+[^\.\n]{0,100})|"
    r"(?:classified\s+as\s+mild[\/ ,]+moderate[\/ ,]+severe(?:\s+based\s+on"
    r"[^\.\n]{0,100})?)",
    re.I,
)
...
def find_severity_definition_v1(text: str):
    return _collect([SEVERITY_TERM_RE], text)
...
def find_severity_definition_v5(text: str):
    return _collect([TIGHT_TEMPLATE_RE], text)


SEVERITY_DEFINITION_FINDERS: dict[
    str, Callable[[str], list[tuple[int, int, str]]]
] = {
    "v1": find_severity_definition_v1,
    "v2": find_severity_definition_v2,
    "v3": find_severity_definition_v3,
    "v4": find_severity_definition_v4,
    "v5": find_severity_definition_v5,
}
__all__ = [
