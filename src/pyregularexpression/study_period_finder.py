"""
study_period_finder.py – p/r ladder for *study period* calendar windows.

Five variants (v1–v5):

    • v1 – high recall: any date/year range pattern
    • v2 – date range + study-period cue (study/study period/from/between)
      within ±window tokens
    • v3 – only inside a *Study period / Study window* heading block
    • v4 – v2 plus explicit start–end keywords (from/to/between) and month
      name or year range; excludes follow-up traps
    • v5 – tight template: “Study period: Jan 2015–Dec 2019.”, “Data were
      collected between 2000 and 2005”, etc.

Each finder returns list of tuples:
(start_token_idx, end_token_idx, snippet)
"""
...
DATE_RANGE_RE = re.compile(
    rf"(?:{MONTH_YEAR}|{YEAR})\s*(?:–|-|to|through|and|until)\s*"
    rf"(?:{MONTH_YEAR}|{YEAR})",
    re.I,
)

STUDY_TERM_RE = re.compile(
    r"\b(?:study\s+period|study\s+window|data\s+collection|study\s+years?|"
    r"between|from)\b",
    re.I,
)
...
HEADING_STUDY_RE = re.compile(
    r"(?m)^(?:study\s+period|study\s+window|data\s+collection\s+period)\s*"
    r"[:\-]?\s*$",
    re.I,
)
...
TIGHT_TEMPLATE_RE = re.compile(
    r"(?:study\s+period|data\s+collection)\s*[:\-]?\s+[^\.\n]{0,60}(?:\d{4}|"
    r"Jan)\b[^\.\n]{0,60}",
    re.I,
)
