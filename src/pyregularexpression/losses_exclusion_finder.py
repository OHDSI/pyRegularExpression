"""
losses_exclusion_finder.py – p/r ladder for *losses and exclusions*.

Five variants (v1–v5):
    • v1 – high recall: any dropout/loss cue (lost to follow-up, withdrew
      consent, dropped out, excluded from analysis) plus a number.
    • v2 – v1 **and** follow‑up/analysis stage keyword within ±4 tokens.
    • v3 – only inside a *Participant Flow / Losses* heading block (first ~500
      characters).
    • v4 – v2 plus explicit reason phrase (due to, because of, adverse event,
      side effects) in same sentence.
    • v5 – tight template: “5 lost to follow-up, 2 withdrew due to side
      effects.”
...
LOSS_CUE_RE = re.compile(
    r"\b(?:lost\s+to\s+follow[- ]up|withdrew|withdrawn|dropped?\s+out|"
    r"drop[- ]outs?|excluded\s+from\s+analysis|missing\s+data)\b",
    re.I,
)
STAGE_RE = re.compile(
    r"\b(?:follow[- ]up|analysis|study\s+period|treatment|intervention)\b", re.I
)
REASON_RE = re.compile(
    r"\b(?:due\s+to|because\s+of|adverse\s+event|side\s+effects?|pregnancy)\b",
    re.I,
)
HEAD_LOSS_RE = re.compile(
    r"(?m)^(?:losses?\s+and\s+exclusions?|drop[- ]?outs?|participant\s+flow)"
    r"\s*[:\-]?.*$",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    rf"{NUM_RE}\s+lost\s+to\s+follow[- ]up,?\s+{NUM_RE}\s+withdrew\s+(?:due\s+to|"
    r"because\s+of)\s+[^\.\n]+",
    re.I,
)
TRAP_RE = re.compile(
    r"\bexcluded\s+during\s+screening|lost\s+samples?|specimens\b", re.I
)
