"""covariate_adjustment_finder.py – precision/recall ladder for *covariate adjustment*
statements.
Five variants (v1–v5):
    ...
    • v4 – v2 plus explicit covariate keyword (age, sex, BMI, smoking, etc.),
           excluding dose‑adjustment traps.
    • v5 – tight template: “HRs adjusted for age, BMI, smoking”,
           “Multivariable model controlling for age and sex”, etc.
...
# 1.  Regex assets
# ─────────────────────────────
ADJUST_VERB_RE = re.compile(
    r"\b(?:adjusted|adjusting|controlling|controlled|accounted)\b", re.I
)

LINK_TOKEN_RE = re.compile(r"\b(?:for|with|including)\b", re.I)

COVARIATE_KEY_RE = re.compile(
    r"\b(?:age|sex|gender|bmi|body\s+mass\s+index|smok(?:e|ing)|race|ethnicity|"
    r"comorbidit(?:y|ies)|diabetes|hypertension|bp|cholesterol|income|education|"
    r"baseline)\b",
    re.I,
)
...
HEADING_ADJ_RE = re.compile(
    r"(?m)^(?:statistical\s+analysis|covariate\s+adjustment|analytical\s+approach)"
    r"\s*[:\-]?\s*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:dose|doses|drug|drugs|treatment|treatments|therapy|therapies|regimen|"
    r"regimens)\b.{0,40}\badjust(?:ment|ed)\b",
    re.I,
)


TIGHT_TEMPLATE_RE = re.compile(
    r"(?:adjusted|controlling|controlled)\s+for\s+[A-Za-z0-9,\s]+(?:age|sex|bmi|smok)"
...
def find_covariate_adjustment_v2(
    text: str, window: int = 4
) -> list[tuple[int, int, str]]:
    ...
    for m in ADJUST_VERB_RE.finditer(text):
        if TRAP_RE.search(text[max(0, m.start() - 40) : m.end() + 40]):
            continue
        w_s, w_e = _char_span_to_word_span((m.start(), m.end()), token_spans)
        if any(link for link in link_idx if w_s - window <= link <= w_e + window):
            out.append((w_s, w_e, m.group(0)))
    return out


def find_covariate_adjustment_v3(
...
def find_covariate_adjustment_v4(
    ...
    tokens = [text[s:e] for s, e in token_spans]
    cov_idx = {
        i for i, t in enumerate(tokens) if COVARIATE_KEY_RE.search(t.strip(",.;:"))
    }
...
# 4.  Mapping & exports
# ─────────────────────────────
COVARIATE_ADJUSTMENT_FINDERS: dict[
