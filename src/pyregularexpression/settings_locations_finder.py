"""
settings_locations_finder.py – p/r ladder for *study settings/locations*.

(where the research was conducted, e.g., hospital, clinic, city, country).

Five variants (v1–v5):
    • v1 – high recall: any clause containing 'setting(s)' or
      'conducted/performed/carried out' + location terms.
    • v2 – v1 **and** explicit facility or geographic term (hospital, clinic,
      university, school, community, country, city, region).
    • v3 – only inside a *Study Setting*, *Methods*, or *Participants* heading
      block (first ~400 characters).
    • v4 – v2 plus geopolitical or institutional entity (country names,
      cities, WHO regions, universities) in same sentence.
    • v5 – tight template: “The study was conducted at [facility] in [city],
      [country].”
...
CONDUCT_RE = re.compile(
    r"\b(?:conducted|performed|carried\s+out|undertaken)\b", re.I
)

FACILITY_RE = re.compile(
    r"\b(?:hospital(?:s)?|clinic(?:s)?|centre(?:s)?|center(?:s)?|university(?:ies)?|"
    r"school(?:s)?|practice(?:s)?|ward(?:s)?|laborator(?:y|ies)|community|communities)\b",
    re.I,
)

GEO_RE = re.compile(
    r"\b(?:USA|United\s+States|UK|United\s+Kingdom|India|China|Canada|Australia|"
    r"Europe|Asia|Africa|France|Germany|Italy|Spain|Brazil|Mexico|Japan|Korea|"
    r"Russia|Nigeria|South\s+Africa|city|country|region)\b\.?",
    re.I,
)

HEAD_SEC_RE = re.compile(
    r"(?i)(study\s+setting|methods?|participants?)\s*[:\-]?", re.M
)

TIGHT_TEMPLATE_RE = re.compile(
    r"(?:the\s+study\s+was\s+(?:conducted|performed|carried\s+out)\s+at\s+.+?\s+"
    r"(?:hospital|clinic|university|school)\s+in\s+[A-Z][a-z]+(?:,\s*[A-Z][a-z]+)*)",
    re.I,
)
...
def find_settings_location_v1(text: str):
    pattern = re.compile(
        r"(?:study\s+settings?|research\s+setting|study\s+was\s+(?:conducted|"
        r"performed|carried\s+out|undertaken))",
        re.I,
    )
    ...
    for w_s, w_e, snippet in matches:
        ...
        if "without reference to" in before or before.endswith(
            "no"
        ) or before.endswith("without"):
            continue
...
# --- Registry ---
SETTINGS_LOCATION_FINDERS: dict[
