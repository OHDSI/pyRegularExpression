"""
outcome_endpoints_finder.py – p/r ladder for *outcomes / endpoints*.

Five variants (v1–v5):
    • v1 – high recall: any outcome cue (primary outcome was, secondary
      endpoints included, outcome measures, endpoint defined as, assessed X at
      Y months).
    • v2 – outcome cue + measurement verb (measured, assessed, defined) within
      ±4 tokens **or** explicit time point (at X months/weeks).
    • v3 – only inside an *Outcome(s) / Endpoints / Outcome Measures* heading
      block (first ~500 characters).
    • v4 – v2 plus distinction of primary vs secondary (both terms or ordinal
      adjectives) in same sentence for precision.
    • v5 – tight template: “Primary outcome was progression‑free survival at
      12 months; secondary outcomes included overall survival.”
...
OUTCOME_CUE_RE = re.compile(
    r"\b(?:primary|secondary)\s+outcomes?\b|\b(?:primary|secondary)\s+endpoints?"
    r"\b|\boutcomes?\s+measures?\b|\bendpoints?\s+defined\s+as\b",
    re.I,
)
MEASURE_VERB_RE = re.compile(
    r"\b(?:was|were|measured|assessed|defined|evaluated|collected)\b", re.I
)
TIME_CUE_RE = re.compile(r"\bat\s+\d+\s*(?:days?|weeks?|months?|years?)\b", re.I)
HEADING_OUT_RE = re.compile(
    r"(?m)^(?:outcomes?|endpoints?|outcome\s+measures?)\s*[:\-]?\s*$", re.I
)
TRAP_RE = re.compile(
    r"\boutcome\s+of\s+the\s+procedure|good\s+outcome|clinical\s+outcome\s+was\s+"
    r"successful\b",
    re.I,
)
TIGHT_TEMPLATE_RE = re.compile(
    r"primary\s+(?:outcome|endpoint)\s+was\s+[^\.\n]{0,80}?\bat\s+\d+\s*(?:months?|"
    r"years?|weeks?)\b[^\.\n]{0,120}?secondary\s+(?:outcomes?|endpoints?)\s+"
    r"included\b",
    re.I,
)
PRIMARY_RE = re.compile(r"\bprimary\s+(?:outcome|endpoint)\b", re.I)
SECONDARY_RE = re.compile(
    r"\bsecondary\s+(?:outcome|endpoint|outcomes|endpoints)\b", re.I
)
...
def find_outcome_endpoints_v2(text: str, window: int = 4):
    ...
    verb_idx = {
        ...
        if MEASURE_VERB_RE.fullmatch(t) or TIME_CUE_RE.fullmatch(t)
    }
    out = []
...
OUTCOME_ENDPOINTS_FINDERS: dict[
