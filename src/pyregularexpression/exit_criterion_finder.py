"""
exit_criterion_finder.py – p/r ladder for *exit-criterion* statements.

Five variants (v1–v5):

    • v1 – high recall: any exit/censoring cue
    • v2 – cue + temporal keyword (‘until’, ‘whichever’, ‘censored at/when’)
      within context window
    • v3 – only inside an *Exit criteria / Censoring* heading block
    • v4 – v2 plus explicit event/time token (death/transplant/date),
      excludes attrition traps
...
EXIT_CRITERION_TERM_RE = re.compile(
    r"\b(?:followed\s+until|until\s+the\s+earlier\s+of|until|exit\s+when|"
    r"censored\s+(?:at|on|when)|exit\s+at|follow[- ]?up\s+ended)\b",
    re.I,
)

TEMPORAL_KEYWORD_RE = re.compile(
    r"\b(?:until|whichever|earlier|later|censored|exit)\b", re.I
)

EVENT_TOKEN_RE = re.compile(
    r"\b(?:death|transplant|end[-\s]+of[-\s]+study|\d{4}-\d{2}-\d{2}|"
    r"31\s+dec\s+\d{4}|\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4})\b",
    re.I,
)

HEADING_EXIT_RE = re.compile(
    r"(?m)^(?:exit\s+criteria|censoring|follow[- ]?up\s+end|end\s+of\s+"
    r"follow[- ]?up)\s*[:\-]?\s*$",
    re.I,
)

TRAP_RE = re.compile(
    r"\b(?:study\s+ended|lost\s+to\s+follow[- ]?up|withdrew|withdrawn|"
    r"dropped\s+out|attrition|analysis)\b",
    re.I,
)
