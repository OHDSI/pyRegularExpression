"""
similarity_of_interventions_finder.py – p/r ladder for *similarity*.

Five variants (v1–v5):
    • v1 – high recall: any similarity cue (identical placebo, matched
      placebo, indistinguishable, sham procedure, double‑dummy).
    • v2 – similarity cue + intervention form word (placebo, capsule,
      injection, device, procedure) within ±4 tokens.
    • v3 – only inside a *Similarity of Interventions* heading block.
    • v4 – v2 plus explicit qualifier (identical, matched, indistinguishable)
      and form word in same context.
    • v5 – tight template: “Control arm received placebo injection identical in
      appearance to active drug.”
...
SIM_CUE_RE = re.compile(
    ...
    r"(?:[^\.\n]{0,20}?\b(?:placebo|capsule|tablet|injection|procedure|device|"
    r"patch|solution|syringe))?",
    re.I,
)
FORM_RE = re.compile(
    r"\b(?:placebo|capsule|tablet|injection|solution|suspension|device|procedure|"
    r"patch|syringe)s?\b",
    re.I,
)
QUAL_RE = re.compile(r"\b(?:identical|matched|indistinguishable)s?\b", re.I)
HEAD_SIM_RE = re.compile(
    r"(?m)^(?:similarity\s+of\s+interventions?|blinding\s+materials?|"
    r"manufacturing\s+matching)\s*[:\-]?\s*$",
    re.I,
)
TRAP_RE = re.compile(r"\bsimilar\s+in\s+(?:duration|effect|class)\b", re.I)
TIGHT_TEMPLATE_RE = re.compile(
    r"placebo\s+(?:capsule|tablet|injection|solution)\s+identical\s+(?:in\s+"
    r"appearance\s+to|to)\s+(?:active|study)\s+(?:drug|treatment)",
    re.I,
)
...
def find_similarity_of_interventions_v4(text: str, window: int = 6):
    ...
    for w_s, w_e, snip in matches:
        ...
        has_qualifier = any(
            QUAL_RE.fullmatch(clean_tokens[i]) for i in range(start, end)
        )
        ...
        negated = any(
            tok in {"no", "not", "without", "none"}
            for tok in window_tokens_lower[:cue_pos_in_window]
        )
...
SIMILARITY_OF_INTERVENTIONS_FINDERS: dict[
...
__all__ = [
    ...
    "SIMILARITY_OF_INTERVENTIONS_FINDERS",
]

find_similarity_of_interventions_high_recall = find_similarity_of_interventions_v1
find_similarity_of_interventions_high_precision = (
    find_similarity_of_interventions_v5
)
